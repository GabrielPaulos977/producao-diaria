    ...
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
        ...
        return (
          <div style={{ padding: "12px 14px 100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              {!isGestor && <button onClick={() => setRetForm({ eqId: "", data: dataSel, notaId: "", items: [], obs: "" })} style={{ padding: "7px 14px", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Registrar</button>}
            </div>
      ...
      {screen === "retrabalho" && retForm && (() => {
        ...
        const addRetItem = (pontoNome) => {
          ...
          setRetForm(f => ({ ...f, items: [...(f.items || []), { pontoNome, motivo: "", qtd: 1 }] }));
        };
        ...
        const canSave = retForm.eqId && retForm.notaId && retItems.length > 0 && retItems.every(i => i.motivo);
        return (
          <div style={{ padding: "14px 14px 100px" }}>
                  ...
                  <div key={item.pontoNome} style={{ background: "#111d33", borderRadius: 10, padding: 10, marginBottom: 6, border: "1px solid #1e2d48" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {MOTIVOS_RETRAB.map(m => (
                        <button key={m} onClick={() => updateRetItem(item.pontoNome, "motivo", m)}
                          style={{ padding: "5px 8px", background: item.motivo === m ? "rgba(239,68,68,.15)" : "#0b1121", border: item.motivo === m ? "1.5px solid rgba(239,68,68,.3)" : "1px solid #1e2d48", borderRadius: 6, cursor: "pointer", fontSize: 9, fontWeight: item.motivo === m ? 700 : 500, color: item.motivo === m ? "#ef4444" : "#5a7aa0" }}>{m}</button>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            <button onClick={() => {
              ...
              retItems.forEach(item => {
                fbPush("retrab", { eqId: retForm.eqId, data: retForm.data, notaId: retForm.notaId, pontoNome: item.pontoNome, qtd: item.qtd, motivo: item.motivo, obs: retForm.obs || "" });
              });
              setRetForm(null);
            }} disabled={!canSave} style={{ width: "100%", padding: "13px 0", marginTop: 8, background: !canSave ? "#1e2d48" : "linear-gradient(135deg,#ef4444,#dc2626)", color: !canSave ? "#3d4d66" : "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Registrar {retItems.length} retrabalho(s)</button>
          </div>
        );
      })()}
      ...
      {screen === "import" && (
        <div style={{ padding: "14px 14px 100px" }}>
          ...
          <button onClick={() => { fbRemove("notas"); fbRemove("atribs"); fbRemove("retrab"); fbRemove("cavas"); fbRemove("preps"); fbRemove("inspecoes"); setImportMsg("✅ Banco resetado! Reimporte os Excel."); }} style={{ width: "100%", marginTop: 14, padding: "12px 0", background: "rgba(239,68,68,.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,.25)", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🗑️ Resetar Banco Completo</button>
        </div>
      )}
