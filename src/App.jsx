...
import { BASES, EQUIPES_POR_BASE, INTEGRANTES_POR_TIPO, FUNCIONARIOS, VEICULOS, INIT_NOTAS, DIAS_UTEIS, DIVISOR_US, MOTIVOS_RETRAB, SERVICOS_LISTA } from "./data";
...
const tipoCor = t => (t === "B3" ? "#3b9eff" : t === "C1" ? "#a78bfa" : t === "B1" ? "#fbbf24" : t === "Polidril" ? "#f97316" : "#475569");
...
export default function App() {
  ...
  const [funcRaw] = useFBData(bp ? `${bp}/funcionarios` : null, null);
  const [ocorrenciasRaw] = useFBData(bp ? `${bp}/ocorrencias` : null, {});
  const [progRaw] = useFBData(bp ? `${bp}/programacao` : null, {});
  const [atrasosRaw] = useFBData(bp ? `${bp}/atrasos` : null, {});
  const [idasRaw] = useFBData(bp ? `${bp}/idas` : null, {});

  // Coordenador: load all bases data
  const [allBasesRaw] = useFBData(role === "coordenador" ? "bases" : null, {});
  ...
  const ocorrencias = fbToArr(ocorrenciasRaw);
  const [ocorrenciaForm, setOcorrenciaForm] = useState(null);
  const atrasos = fbToArr(atrasosRaw);
  const idas = fbToArr(idasRaw);
  const [idaForm, setIdaForm] = useState(null);
  const [notaDetalhe, setNotaDetalhe] = useState(null);
  const [atrasoForm, setAtrasoForm] = useState(null);
  
  // Funcionarios: Firebase with static fallback + auto-seed
  const funcionarios = useMemo(() => {
    if (funcRaw === null || funcRaw === undefined) return FUNCIONARIOS[baseSel] || [];
    if (Array.isArray(funcRaw)) return funcRaw.filter(f => f);
    if (typeof funcRaw === "object" && Object.keys(funcRaw).length > 0) return Object.values(funcRaw).filter(f => f);
    return FUNCIONARIOS[baseSel] || [];
  }, [funcRaw, baseSel]);
  
  useEffect(() => {
    if (bp && funcRaw !== null && funcRaw !== undefined && (Array.isArray(funcRaw) ? funcRaw.filter(f=>f).length === 0 : Object.keys(funcRaw || {}).length === 0)) {
      const sf = FUNCIONARIOS[baseSel] || [];
      if (sf.length > 0) fbSet(bp + "/funcionarios", sf);
    }
  }, [bp, funcRaw, baseSel]);
  ...
  // Lookup municipio/prazo from programming data
  const getNotaInfo = useCallback((notaNome) => {
    const prog = progRaw || {};
    for (const dt of Object.keys(prog)) {
      const dayData = prog[dt];
      if (!dayData?.notas) continue;
      const notasList = Array.isArray(dayData.notas) ? dayData.notas : Object.values(dayData.notas);
      const found = notasList.find(n => n.nota === notaNome || notaNome?.includes(n.nota) || n.nota?.includes(notaNome));
      if (found) return { municipio: found.municipio || "", prazo: found.prazo || "", regulatorio: found.regulatorio || "" };
    }
    return { municipio: "", prazo: "", regulatorio: "" };
  }, [progRaw]);

  // Cava value: SOLO C teams (SRO 7744, SRO 7745) = 22.07 US, others = 2 US
  const getCavaValue = useCallback((prepEqId) => {
    const eq = EQUIPES.find(e => e.id === prepEqId);
    if (eq && (eq.nome.includes("7744") || eq.nome.includes("7745"))) return 22.07;
    return 2;
  }, [EQUIPES]);
  ...
  const getAtribReal = useCallback((a) => {
    ...
    const aCavas = cavas.filter(c => c.atribId === (a._fbKey || a.id));
    const cavaUSTotal = aCavas.reduce((s, c) => s + getCavaValue(c.prepEqId), 0);
    return { realR: cR + eR - cavaUSTotal * DIVISOR_US, realUS: cU + eR / DIVISOR_US - cavaUSTotal };
  }, [getPts, cavas, getCavaValue]);

  const getCavaUS = useCallback((eqId, filterFn) => cavas.filter(c => c.prepEqId === eqId && filterFn(c)).reduce((s, c) => s + getCavaValue(c.prepEqId), 0), [cavas, getCavaValue]);
  ...
  const togglePonto = (aKey, pId) => {
    ...
    if (next === "ok") {
      ...
      if (pt && pt.n.toUpperCase().startsWith("P")) {
        st[pId] = next;
        fbSet(bp+`/atribs/${aKey}/status`, st);
    ...
    if (cur === "ok") {
      ...
      cavas.filter(c => c.atribId === aKey && c.pontoId === pId).forEach(c => fbRemove(bp+`/cavas/${c._fbKey}`));
    }
    st[pId] = next;
    fbSet(bp+`/atribs/${aKey}/status`, st);
  };
  ...
  const addExtra = (aKey, desc, valor) => {
    ...
    fbSet(bp+`/atribs/${aKey}/extras`, extras);
  };
  ...
  const delAtrib = (aKey) => { fbRemove(bp+`/atribs/${aKey}`); };
  ...
  const ranking = useMemo(() => {
    return EQUIPES.filter(eq => eq.meta > 0).map(eq => {
  ...
  const mesTotalRealUS = useMemo(() => {
    let t = 0;
    EQUIPES.filter(eq => eq.meta > 0).forEach(eq => { t += getEqMonthUS(eq.id, histMonth); });
  ...
  if (!role || (!baseSel && role !== "coordenador" && role !== "programador")) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,#060d1a 0%,#0b1121 30%,#0f1a2e 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',system-ui,sans-serif", position: "relative", overflow: "hidden" }}>
        ...
        {/* Background electric grid */}
        <div style={{ position: "absolute", inset: 0, opacity: .04, backgroundImage: "radial-gradient(circle at 25% 25%, #eab308 1px, transparent 1px), radial-gradient(circle at 75% 75%, #3b9eff 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
        <div style={{ position: "absolute", top: "10%", left: "50%", transform: "translateX(-50%)", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, rgba(234,179,8,.06) 0%, transparent 70%)", animation: "electricPulse 4s ease infinite" }} />
        
        <div className="fade-in" style={{ textAlign: "center", padding: 30, width: "100%", maxWidth: 360, position: "relative", zIndex: 1 }}>
          <div style={{ display: "inline-block", padding: 16, background: "rgba(255,255,255,.03)", borderRadius: 20, marginBottom: 16, border: "1px solid rgba(234,179,8,.1)" }}>
            <img src="/Procel.jpeg" alt="Procellecorp" style={{ width: 200, display: "block", borderRadius: 12 }} />
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: "#f8fafc", marginBottom: 2, letterSpacing: -.5 }}>Indicadores Procel</h1>
          <p style={{ fontSize: 11, color: "#eab308", fontWeight: 600, marginBottom: 6, letterSpacing: 2, textTransform: "uppercase" }}>⚡ Controle BT / MT</p>
          <p style={{ fontSize: 10, color: "#3a5070", marginBottom: 28 }}>Sistema de Gestão de Produção Elétrica</p>

          {!role ? (
            <>
              <button onClick={() => setRole("auxiliar")} style={{ width: "100%", padding: "14px 24px", marginBottom: 8, background: "linear-gradient(135deg,#eab308,#d97706)", color: "#0b1121", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 20px rgba(234,179,8,.25)", transition: "all .2s" }}>⚡ Auxiliar Técnico</button>
              <div style={{ fontSize: 10, color: "#4b6080", marginBottom: 18 }}>Produção, pontos, retrabalho, preparação</div>

              <button onClick={() => setRole("gestor")} style={{ width: "100%", padding: "14px 24px", marginBottom: 8, background: "linear-gradient(135deg,rgba(59,158,255,.1),rgba(59,158,255,.05))", color: "#60a5fa", border: "1px solid rgba(59,158,255,.2)", borderRadius: 14, fontSize: 15, fontWeight: 800, cursor: "pointer", transition: "all .2s" }}>👔 Gerente</button>
              <div style={{ fontSize: 10, color: "#4b6080", marginBottom: 18 }}>Gestão completa, indicadores, ranking</div>

              <button onClick={() => { setRole("coordenador"); setBaseSel("all"); setScreen("visao_geral"); }} style={{ width: "100%", padding: "14px 24px", background: "linear-gradient(135deg,rgba(167,139,250,.1),rgba(167,139,250,.05))", color: "#a78bfa", border: "1px solid rgba(167,139,250,.2)", borderRadius: 14, fontSize: 15, fontWeight: 800, cursor: "pointer", transition: "all .2s" }}>👑 Coordenador</button>
              <div style={{ fontSize: 10, color: "#4b6080", marginTop: 6, marginBottom: 16 }}>Visão consolidada de todas as bases</div>

              <button onClick={() => { setRole("programador"); setBaseSel("all"); setScreen("prog_import"); }} style={{ width: "100%", padding: "12px 24px", background: "rgba(34,211,238,.04)", color: "#22d3ee", border: "1px dashed rgba(34,211,238,.2)", borderRadius: 14, fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all .2s" }}>⚙️ Programador</button>
  ...
  const tabs = isProgramador
    ...
    : isCoordenador
    ? [{ k: "visao_geral", l: "Visão Geral" }, { k: "relatorio", l: "Relatório" }, { k: "programacao_dia", l: "Programação" }, { k: "seguranca_coord", l: "Segurança" }]
    : isGestor
    ? [{ k: "home", l: "Painel" }, { k: "gerencial", l: "Gerencial" }, { k: "programacao_ger", l: "Programação" }, { k: "ranking", l: "Ranking" }, { k: "status", l: "Status" }, { k: "seguranca", l: "Segurança" }, { k: "qualidade", l: "Qualidade" }, { k: "preparacao_view", l: "Prepar." }, { k: "import", l: "Importar" }]
    : [{ k: "home", l: "Painel" }, { k: "ranking", l: "Ranking" }, { k: "status", l: "Status" }, { k: "preparacao_view", l: "Prepar." }, { k: "import", l: "Importar" }];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800;900&family=JetBrains+Mono:wght@500;600;700;800&display=swap');*{box-sizing:border-box;margin:0}input,select,button{font-family:inherit}::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#2d3d56;border-radius:3px}.m{font-family:'JetBrains Mono',monospace}
      @keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
      @keyframes slideIn{from{opacity:0;transform:translateX(-10px)}to{opacity:1;transform:translateX(0)}}
      @keyframes pulseGlow{0%,100%{box-shadow:0 0 8px rgba(234,179,8,.15)}50%{box-shadow:0 0 20px rgba(234,179,8,.3)}}
      @keyframes electricPulse{0%,100%{opacity:.6}50%{opacity:1}}
      .fade-in{animation:fadeIn .4s ease-out both}
      .slide-in{animation:slideIn .3s ease-out both}
      .glass{background:rgba(17,29,51,.6)!important;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid rgba(59,158,255,.1)!important}
      .glass-card{background:linear-gradient(135deg,rgba(13,24,41,.8),rgba(17,29,51,.6))!important;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.05)!important;transition:all .3s ease}
      .glass-card:hover{border-color:rgba(234,179,8,.2)!important;box-shadow:0 4px 20px rgba(0,0,0,.3)}
      .glow-border{box-shadow:0 0 0 1px rgba(59,158,255,.1),0 4px 16px rgba(0,0,0,.2)}
      .tab-active{background:linear-gradient(135deg,#eab308,#d97706)!important;color:#0b1121!important;box-shadow:0 2px 12px rgba(234,179,8,.3)}
      .btn-primary{background:linear-gradient(135deg,#eab308,#d97706)!important;color:#0b1121!important;box-shadow:0 4px 16px rgba(234,179,8,.25);transition:all .2s}
      .btn-primary:hover{box-shadow:0 6px 24px rgba(234,179,8,.4);transform:translateY(-1px)}
      `}</style>
      <style>{`
        @media(min-width:768px){
          .resp-content{max-width:800px;margin:0 auto!important}
        }
        @media(min-width:1200px){
          .resp-content{max-width:1000px}
        }
      `}</style>

      {/* Header */}
      <div style={{ background: "linear-gradient(135deg,#080e1a,#0f1a2e,#162240)", padding: "12px 16px", borderBottom: "1px solid rgba(234,179,8,.15)", boxShadow: "0 2px 16px rgba(0,0,0,.3)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ position: "relative" }}>
              <img src="/Procel.jpeg" alt="Procel" style={{ width: 38, height: 38, borderRadius: 10, objectFit: "contain", background: "#fff", padding: 2 }} />
              <div style={{ position: "absolute", bottom: -2, right: -2, width: 10, height: 10, borderRadius: 5, background: "#34d399", border: "2px solid #0f1a2e" }} />
            </div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 900, color: "#f8fafc", letterSpacing: -.3 }}>Indicadores Procel</div>
              <div style={{ fontSize: 9, color: "#4b6080", fontWeight: 600 }}>
                ...
                {alertasNaoLidos.length > 0 && <span style={{ marginLeft: 6, padding: "1px 6px", background: "#ef4444", color: "#fff", borderRadius: 10, fontSize: 9, fontWeight: 800, animation: "electricPulse 2s infinite" }}>🔔 {alertasNaoLidos.length}</span>}
              </div>
            </div>
          </div>
      ...
      <div style={{ display: "flex", background: "#0f1525", borderBottom: "1px solid #1a2236", overflowX: "auto" }}>
        {tabs.map(t => (
          <button key={t.k} onClick={() => setScreen(t.k)} style={{ flex: "1 0 auto", padding: "10px 10px", fontSize: 10, fontWeight: 700, border: "none", cursor: "pointer", background: screen === t.k ? "linear-gradient(135deg,rgba(234,179,8,.15),rgba(234,179,8,.05))" : "transparent", color: screen === t.k ? "#eab308" : "#4b6080", borderBottom: screen === t.k ? "2px solid #eab308" : "2px solid transparent", whiteSpace: "nowrap", borderRadius: screen === t.k ? "8px 8px 0 0" : "0", transition: "all .2s", letterSpacing: .3 }}>{t.l}</button>
        ))}
      </div>
      ...
      {screen === "prog_import" && isProgramador && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
      ...
      {screen === "relatorio" && isCoordenador && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "0 0 100px", background: "#080e1a" }}>
            {/* Report Header */}
      ...
      {screen === "programacao_dia" && isCoordenador && (() => {
        ...
        const [progPeriodo, setProgPeriodo] = [statusFilter, setStatusFilter];
        
        // Period calculations (parse as local time)
        const [ty, tm, td] = today.split("-").map(Number);
        const todayDate = new Date(ty, tm - 1, td);
        todayDate.setHours(12, 0, 0, 0);
        const dayOfWeek = todayDate.getDay();
        const mondayThis = new Date(todayDate); mondayThis.setDate(todayDate.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1));
        ...
        const mesInicio = new Date(todayDate.getFullYear(), todayDate.getMonth(), 1);
        const mesFim = new Date(todayDate.getFullYear(), todayDate.getMonth() + 1, 0);
        
        const fmtDate = (d) => d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
        
        const periodos = {
          "hoje": { label: "Hoje", inicio: today, fim: today },
          "esta_semana": { label: "Esta Semana", inicio: fmtDate(mondayThis), fim: fmtDate(sundayThis) },
          "semana_passada": { label: "Sem. Passada", inicio: fmtDate(mondayLast), fim: fmtDate(sundayLast) },
          "proxima_semana": { label: "Próx. Semana", inicio: fmtDate(mondayNext), fim: fmtDate(sundayNext) },
          "este_mes": { label: "Este Mês", inicio: fmtDate(mesInicio), fim: fmtDate(mesFim) },
          "personalizado": { label: "Personalizado", inicio: dataSel, fim: dataSel },
        };
        
        const periodo = periodos[progPeriodo] || periodos["hoje"];
        const dataInicio = periodo.inicio;
        const dataFim = periodo.fim;
        
        // Build date range (parse as local time, not UTC)
        const parseLocal = (s) => { const [y, m, dd] = s.split("-").map(Number); return new Date(y, m - 1, dd); };
        const datasRange = [];
        let d = parseLocal(dataInicio);
        const dFim = parseLocal(dataFim);
        while (d <= dFim) { datasRange.push(fmtDate(d)); d.setDate(d.getDate() + 1); }
        
        // Aggregate data across all dates in range per base
        const progDia = BASES.map((base, i) => {
          const bd = allBasesRaw?.[base.id] || {};
          const allProg = bd.programacao || {};
          let allNotas = [];
          datasRange.forEach(dt => {
            const dayProg = allProg[dt];
            if (dayProg) {
              const notasList = Array.isArray(dayProg.notas) ? dayProg.notas : Object.values(dayProg.notas || {});
              notasList.forEach(n => allNotas.push({ ...n, dia: dt }));
            }
          });
          
          const filtered = progSearch
            ? allNotas.filter(n => [n.nota, n.municipio, n.regulatorio, n.status].join(" ").toLowerCase().includes(progSearch.toLowerCase()))
            : allNotas;
          
          return { ...base, color: BASE_COLORS[i], notas: filtered, totalUS: filtered.reduce((s, n) => s + (Number(n.us) || 0), 0), qtdNotas: filtered.length, allNotas: allNotas.length, allUS: allNotas.reduce((s, n) => s + (Number(n.us) || 0), 0) };
        });
        
        const totalNotas = progDia.reduce((s, b) => s + b.qtdNotas, 0);
        const totalUS = progDia.reduce((s, b) => s + b.totalUS, 0);
        const diasNoPeriodo = datasRange.length;
        const mediaUSDia = diasNoPeriodo > 0 ? totalUS / diasNoPeriodo : 0;
        
        // Daily chart data
        const dailyChartData = datasRange.map(dt => {
          let usDay = 0, notasDay = 0;
          BASES.forEach(base => {
            const bd = allBasesRaw?.[base.id] || {};
            const dayProg = bd.programacao?.[dt];
            if (dayProg) {
              const nl = Array.isArray(dayProg.notas) ? dayProg.notas : Object.values(dayProg.notas || {});
              notasDay += nl.length;
              usDay += nl.reduce((s, n) => s + (Number(n.us) || 0), 0);
            }
          });
          return { dia: dt.slice(8), data: dt, us: Math.round(usDay), notas: notasDay };
        });
        
        // Status & regulatório breakdowns
        const statusCount = {};
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 12 }}>📋 Programação</h2>
            
            {/* Period buttons */}
            <div style={{ display: "flex", gap: 4, marginBottom: 10, overflowX: "auto" }}>
              {Object.entries(periodos).map(([key, p]) => (
                <button key={key} onClick={() => setProgPeriodo(key)} style={{ flex: "0 0 auto", padding: "8px 12px", background: progPeriodo === key ? "linear-gradient(135deg,#eab308,#d97706)" : "#111d33", color: progPeriodo === key ? "#0b1121" : "#5a7aa0", border: progPeriodo === key ? "none" : "1px solid #1e2d48", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{p.label}</button>
              ))}
            </div>
            
            {/* Custom date range for personalizado */}
            {progPeriodo === "personalizado" && (
            ...
            {/* Period info */}
            <div style={{ fontSize: 10, color: "#5a7aa0", marginBottom: 12, textAlign: "center" }}>
              {dataInicio === dataFim
                ? <span>{dataInicio.split("-").reverse().join("/")}</span>
                : <span>{dataInicio.split("-").reverse().join("/")} → {dataFim.split("-").reverse().join("/")} <strong style={{ color: "#eab308" }}>({diasNoPeriodo} dias)</strong></span>
              }
            </div>
            
            {/* KPIs */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              <div style={{ background: "#111d33", borderRadius: 12, padding: "12px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #eab308" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Notas</div>
                <div className="m" style={{ fontSize: 24, fontWeight: 800, color: "#eab308" }}>{totalNotas}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 12, padding: "12px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #34d399" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Total US</div>
                <div className="m" style={{ fontSize: 24, fontWeight: 800, color: "#34d399" }}>{Math.round(totalUS)}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 12, padding: "12px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #3b9eff" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Média/dia</div>
                <div className="m" style={{ fontSize: 24, fontWeight: 800, color: "#3b9eff" }}>{Math.round(mediaUSDia)}</div>
              </div>
            </div>
            ...
            {/* Daily chart - US per day (only for multi-day periods) */}
            {dailyChartData.length > 1 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>US Programados por Dia</div>
                <ResponsiveContainer width="100%" height={180}>
                  <ComposedChart data={dailyChartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    ...
                    <YAxis tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} />
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.data?.split("-").reverse().join("/")}</div><div style={{ fontSize: 11, color: "#34d399" }}>{payload[0]?.value} US</div><div style={{ fontSize: 10, color: "#5a7aa0" }}>{payload[0]?.payload?.notas} nota(s)</div></div> : null} />
                    <Bar dataKey="us" radius={[4, 4, 0, 0]} barSize={16}>
                      {dailyChartData.map((d, i) => <Cell key={i} fill={d.us > 0 ? "#eab308" : "#1e2d48"} />)}
                    </Bar>
                    <Line type="monotone" dataKey={() => Math.round(mediaUSDia)} stroke="#3b9eff" strokeWidth={2} strokeDasharray="6 4" dot={false} />
                  </ComposedChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 4 }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 10, borderRadius: 2, background: "#eab308" }}></span>US/dia</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 2, background: "#3b9eff" }}></span>Média</span>
                </div>
              </div>
            )}

            {/* Bar chart - US por Base */}
            {totalNotas > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>US por Base</div>
                <ResponsiveContainer width="100%" height={180}>
      ...
      {screen === "seguranca_coord" && isCoordenador && (() => {
        const fbToArrLocal = (obj) => { if (!obj) return []; if (Array.isArray(obj)) return obj; return Object.entries(obj).map(([k, v]) => ({ ...v, _fbKey: k })); };
        const BASE_COLORS = ["#3b9eff", "#34d399", "#facc15"];
        
        const baseSeguranca = BASES.map((base, i) => {
          const bd = allBasesRaw?.[base.id] || {};
          const bInsp = fbToArrLocal(bd.inspecoes).filter(x => x.data?.startsWith(histMonth));
          const bOcorr = fbToArrLocal(bd.ocorrencias).filter(x => x.data?.startsWith(histMonth));
          
          const totalInsp = bInsp.length;
          const avgComp = totalInsp > 0 ? Math.round(bInsp.reduce((s, x) => s + (Number(x.comportamento) || 0), 0) / totalInsp * 10) / 10 : 0;
          const avgCond = totalInsp > 0 ? Math.round(bInsp.reduce((s, x) => s + (Number(x.condicao) || 0), 0) / totalInsp * 10) / 10 : 0;
          const pontuadas = bInsp.filter(x => (Number(x.comportamento) < 100) || (Number(x.condicao) < 100)).length;
          
          const totalOcorr = bOcorr.length;
          const incidentes = bOcorr.filter(x => x.tipo?.includes("Incidente")).length;
          const acidentes = bOcorr.filter(x => x.tipo?.includes("Acidente")).length;
          
          const tipoCount = {};
          bOcorr.forEach(o => { tipoCount[o.tipo || "Outro"] = (tipoCount[o.tipo || "Outro"] || 0) + 1; });
          
          return { ...base, color: BASE_COLORS[i], totalInsp, avgComp, avgCond, pontuadas, totalOcorr, incidentes, acidentes, tipoCount };
        });
        
        const totalInspGeral = baseSeguranca.reduce((s, b) => s + b.totalInsp, 0);
        const totalOcorrGeral = baseSeguranca.reduce((s, b) => s + b.totalOcorr, 0);
        const totalIncidentes = baseSeguranca.reduce((s, b) => s + b.incidentes, 0);
        const totalAcidentes = baseSeguranca.reduce((s, b) => s + b.acidentes, 0);
        const avgCompGeral = totalInspGeral > 0 ? Math.round(baseSeguranca.reduce((s, b) => s + b.avgComp * b.totalInsp, 0) / totalInspGeral * 10) / 10 : 0;
        const avgCondGeral = totalInspGeral > 0 ? Math.round(baseSeguranca.reduce((s, b) => s + b.avgCond * b.totalInsp, 0) / totalInspGeral * 10) / 10 : 0;
        
        // All occurrences for timeline
        const allOcorr = [];
        BASES.forEach(base => { const bd = allBasesRaw?.[base.id] || {}; fbToArrLocal(bd.ocorrencias).filter(x => x.data?.startsWith(histMonth)).forEach(o => allOcorr.push({ ...o, base: base.nome, baseSigla: base.sigla })); });
        allOcorr.sort((a, b) => (b.data + b.hora).localeCompare(a.data + a.hora));
        
        // Tipo consolidado
        const tipoGeral = {};
        allOcorr.forEach(o => { tipoGeral[o.tipo || "Outro"] = (tipoGeral[o.tipo || "Outro"] || 0) + 1; });

        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 12 }}>🔒 Segurança — Visão Geral</h2>
            <MonthNav value={histMonth} onChange={setHistMonth} style={{ width: "100%", marginBottom: 14 }} />
            
            {/* KPIs consolidados */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                { label: "Inspeções", value: totalInspGeral, color: "#3b9eff" },
                { label: "Ocorrências", value: totalOcorrGeral, color: "#f97316" },
                { label: "Incidentes", value: totalIncidentes, color: "#facc15" },
                { label: "Acidentes", value: totalAcidentes, color: "#ef4444" },
              ].map(k => (
                <div key={k.label} style={{ background: "#111d33", borderRadius: 10, padding: "10px 6px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid " + k.color }}>
                  <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.label}</div>
                  <div className="m" style={{ fontSize: 20, fontWeight: 800, color: k.color }}>{k.value}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              <div style={{ flex: 1, background: "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid #1e2d48" }}>
                ...
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: avgCompGeral >= 100 ? "#34d399" : avgCompGeral >= 90 ? "#facc15" : "#ef4444" }}>{avgCompGeral}%</div>
              </div>
              <div style={{ flex: 1, background: "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid #1e2d48" }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Média Condição</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: avgCondGeral >= 100 ? "#34d399" : avgCondGeral >= 90 ? "#facc15" : "#ef4444" }}>{avgCondGeral}%</div>
              </div>
            </div>
            
            {/* Radar comparativo por base */}
            {totalInspGeral > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 6, marginBottom: 6 }}>Comparativo entre Bases</div>
                <ResponsiveContainer width="100%" height={240}>
                  <RadarChart data={[
                    { criterio: "Comportamento", ...Object.fromEntries(baseSeguranca.map(b => [b.sigla, b.avgComp])) },
                    { criterio: "Condição", ...Object.fromEntries(baseSeguranca.map(b => [b.sigla, b.avgCond])) },
                    { criterio: "Inspeções", ...Object.fromEntries(baseSeguranca.map(b => [b.sigla, Math.min(b.totalInsp * 10, 100)])) },
                    { criterio: "Sem Ocorr.", ...Object.fromEntries(baseSeguranca.map(b => [b.sigla, Math.max(0, 100 - b.totalOcorr * 20)])) },
                  ]}>
                    <PolarGrid stroke="#1e2d48" />
                    <PolarAngleAxis dataKey="criterio" tick={{ fill: "#94a3b8", fontSize: 9 }} />
                    <PolarRadiusAxis angle={90} domain={[0, 100]} tick={false} axisLine={false} />
                    {baseSeguranca.map((b, i) => <Radar key={b.id} name={b.nome} dataKey={b.sigla} stroke={b.color} fill={b.color} fillOpacity={0.1} strokeWidth={2} dot={{ r: 3, fill: b.color }} />)}
                    <Tooltip content={({ active, payload, label }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 10, color: "#f1f5f9", fontWeight: 700, marginBottom: 2 }}>{label}</div>{payload.map((p, i) => <div key={i} style={{ fontSize: 10, color: p.stroke }}>{p.name}: {p.value}</div>)}</div> : null} />
                  </RadarChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", justifyContent: "center", gap: 16 }}>
                  {baseSeguranca.map(b => <span key={b.id} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, color: "#94a3b8" }}><span style={{ width: 10, height: 10, borderRadius: 5, background: b.color }}></span>{b.nome}</span>)}
                </div>
              </div>
            )}
            
            {/* Cards por base */}
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 8 }}>Por Base</div>
            {baseSeguranca.map(b => (
              <div key={b.id} style={{ background: "#111d33", borderRadius: 12, padding: "12px 14px", marginBottom: 8, border: "1px solid #1e2d48", borderLeft: "4px solid " + b.color }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: b.color, marginBottom: 8 }}>{b.nome}</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
                  {[
                    { l: "Inspeções", v: b.totalInsp, c: "#3b9eff" },
                    { l: "Pontuadas", v: b.pontuadas, c: b.pontuadas > 0 ? "#ef4444" : "#34d399" },
                    { l: "Ocorrências", v: b.totalOcorr, c: "#f97316" },
                    { l: "Acidentes", v: b.acidentes, c: b.acidentes > 0 ? "#ef4444" : "#34d399" },
                  ].map(k => (
                    <div key={k.l} style={{ textAlign: "center", background: "#0d1525", borderRadius: 6, padding: "6px 2px" }}>
                      <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700 }}>{k.l}</div>
                      <div className="m" style={{ fontSize: 14, fontWeight: 800, color: k.c }}>{k.v}</div>
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                  <div style={{ flex: 1, fontSize: 9, color: "#5a7aa0" }}>Comport.: <strong style={{ color: b.avgComp >= 100 ? "#34d399" : "#facc15" }}>{b.avgComp}%</strong></div>
                  <div style={{ flex: 1, fontSize: 9, color: "#5a7aa0" }}>Condição: <strong style={{ color: b.avgCond >= 100 ? "#34d399" : "#facc15" }}>{b.avgCond}%</strong></div>
                </div>
              </div>
            ))}
            
            {/* Tipos de ocorrência consolidados */}
            {Object.keys(tipoGeral).length > 0 && (
              <div style={{ marginTop: 8, marginBottom: 14 }}>
                <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Tipos de Ocorrência</div>
                {Object.entries(tipoGeral).sort((a, b) => b[1] - a[1]).map(([tipo, count]) => (
                  <div key={tipo} style={{ background: "#111d33", borderRadius: 6, padding: "6px 10px", marginBottom: 3, border: "1px solid #1e2d48", display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 10, color: "#d4dce9" }}>{tipo.includes("Acidente") ? "🔴" : "🟠"} {tipo}</span>
                    <span className="m" style={{ fontSize: 11, fontWeight: 800, color: tipo.includes("Acidente") ? "#ef4444" : "#f97316" }}>{count}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {allOcorr.length > 0 && (
              <div>
                <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Últimas Ocorrências</div>
                {allOcorr.slice(0, 10).map((o, i) => (
                  <div key={o._fbKey || i} style={{ background: "#111d33", borderRadius: 8, padding: "8px 12px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid " + (o.tipo?.includes("Acidente") ? "#ef4444" : "#f97316") }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{o.colaborador}</div>
                        <div style={{ fontSize: 9, color: "#5a7aa0" }}>{o.equipe} · {o.baseSigla} · {o.data?.split("-").reverse().join("/")} {o.hora}</div>
                      </div>
                      <span style={{ fontSize: 8, fontWeight: 700, color: o.tipo?.includes("Acidente") ? "#ef4444" : "#f97316", background: o.tipo?.includes("Acidente") ? "rgba(239,68,68,.1)" : "rgba(249,115,22,.1)", padding: "2px 6px", borderRadius: 4 }}>{o.tipo}</span>
                    </div>
                    <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 2 }}>{o.descricao?.substring(0, 100)}{o.descricao?.length > 100 ? "..." : ""}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      {!notasLoading && screen === "home" && (
        <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
          ...
          <MonthNav value={histMonth} onChange={setHistMonth} style={{ width: "100%", marginBottom: 10 }} />

          {/* Two Speedometers - Produção e Conclusão */}
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <div style={{ flex: 1, background: "linear-gradient(135deg,rgba(13,24,41,.9),rgba(19,32,53,.7))", borderRadius: 16, padding: "16px 8px", border: "1px solid rgba(255,255,255,.05)", boxShadow: "0 4px 20px rgba(0,0,0,.2)" }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, color: "#5a7aa0", textTransform: "uppercase", letterSpacing: .8, marginBottom: 6 }}>Equipes — {Object.keys(eqMap).length}/{EQUIPES.filter(eq => eq.meta > 0).length}</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {EQUIPES.filter(eq => eq.meta > 0).map(eq => {
              const ea = eqMap[eq.id] || [];
              ...
              return (
                <div key={eq.id} onClick={() => { setSelEq(ea); setScreen("detalhe"); }} style={{ background: "linear-gradient(135deg,rgba(13,24,41,.9),rgba(17,29,51,.7))", borderRadius: 14, backdropFilter: "blur(8px)", border: abaixo ? "1.5px solid rgba(239,68,68,.3)" : "1.5px solid " + pctCol(pc) + "30", padding: "10px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, transition: "border-color .2s" }}>
          ...
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              ...
              <select value={histEq} onChange={e => setHistEq(e.target.value)} style={{ flex: 1, ...inp }}>
                ...
                {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}
              </select>
            </div>
            ...
            {(() => {
              ...
              const campo = funcs.filter(f => !["ADM","ALMOX","PATIO","TREINAMENTO_LV"].includes(f.eq));
              const admF = funcs.filter(f => f.eq === "ADM");
              const almoxF = funcs.filter(f => f.eq === "ALMOX");
              const patioF = funcs.filter(f => f.eq === "PATIO");
              const treinF = funcs.filter(f => f.eq === "TREINAMENTO_LV");
              const eqCount = {};
              campo.forEach(f => { eqCount[f.eq] = (eqCount[f.eq] || 0) + 1; });
              const eqEntries = Object.entries(eqCount).sort((a, b) => a[0].localeCompare(b[0]));
              const eqsAtivas = EQUIPES.filter(eq => eq.meta > 0).length;
              const tipoCount = {};
              EQUIPES.filter(eq => eq.meta > 0).forEach(eq => { const t = eq.tipo || "-"; tipoCount[t] = (tipoCount[t] || 0) + 1; });
              const TIPO_COLORS = { "B3": "#34d399", "B2": "#3b9eff", "B1": "#facc15", "C1": "#a78bfa", "Polidril": "#f97316", "-": "#5a7aa0" };
              const allEqs = [...new Set(funcs.map(f => f.eq))].sort();
              const [editIdx, setEditIdx] = [retForm?.editFuncIdx ?? null, (v) => setRetForm(f => f ? { ...f, editFuncIdx: v } : { editFuncIdx: v })];
              const [showAdd, setShowAdd] = [retForm?.showAddFunc || false, (v) => setRetForm(f => f ? { ...f, showAddFunc: v } : { showAddFunc: v })];
              const [newF, setNewF] = [retForm?.newFunc || { eq: "", nome: "", fn: "" }, (v) => setRetForm(f => f ? { ...f, newFunc: v } : { newFunc: v })];
              const saveFuncs = (list) => fbSet(bp + "/funcionarios", list);
              const moverFunc = (idx, novaEq) => { const u = [...funcs]; u[idx] = { ...u[idx], eq: novaEq }; saveFuncs(u); setEditIdx(null); };
              const removerFunc = (idx) => { if (!confirm("Remover " + funcs[idx].nome + "?")) return; saveFuncs(funcs.filter((_, i) => i !== idx)); };
              const adicionarFunc = () => { if (!newF.nome || !newF.eq) return; saveFuncs([...funcs, { ...newF }]); setNewF({ eq: "", nome: "", fn: "" }); setShowAdd(false); };
              return (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 14px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>👥 Efetivo — {baseNome}</span>
                    <button onClick={() => setShowAdd(!showAdd)} style={{ padding: "4px 10px", background: showAdd ? "#ef4444" : "#34d399", color: showAdd ? "#fff" : "#0b1121", border: "none", borderRadius: 6, fontSize: 9, fontWeight: 700, cursor: "pointer" }}>{showAdd ? "✕ Cancelar" : "+ Adicionar"}</button>
                  </div>
                  {showAdd && (<div style={{ background: "#0d1525", borderRadius: 8, padding: 10, marginBottom: 10, border: "1px solid #1a2d4d" }}><select value={newF.eq} onChange={e => setNewF({ ...newF, eq: e.target.value })} style={{ ...inp, marginBottom: 4, fontSize: 11 }}><option value="">Selecione equipe</option>{allEqs.map(eq => <option key={eq} value={eq}>{eq}</option>)}<option value="ADM">ADM</option><option value="ALMOX">ALMOX</option><option value="PATIO">PATIO</option><option value="TREINAMENTO_LV">Treinamento LV</option></select><input value={newF.nome} onChange={e => setNewF({ ...newF, nome: e.target.value })} placeholder="Nome completo" style={{ ...inp, marginBottom: 4, fontSize: 11 }} /><input value={newF.fn} onChange={e => setNewF({ ...newF, fn: e.target.value })} placeholder="Função" style={{ ...inp, marginBottom: 6, fontSize: 11 }} /><button onClick={adicionarFunc} style={{ width: "100%", padding: "8px 0", background: "#34d399", color: "#0b1121", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Adicionar</button></div>)}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr", gap: 5, marginBottom: 10 }}>{[{label:"Campo",value:campo.length,color:"#34d399"},{label:"ADM",value:admF.length,color:"#a78bfa"},{label:"Almox",value:almoxF.length,color:"#facc15"},{label:"Pátio",value:patioF.length,color:"#3b9eff"},{label:"Trein.LV",value:treinF.length,color:"#f97316"},{label:"Total",value:totalFunc,color:"#f1f5f9"}].map(k => (<div key={k.label} style={{ background: "#0d1525", borderRadius: 8, padding: "8px 4px", textAlign: "center" }}><div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.label}</div><div className="m" style={{ fontSize: 16, fontWeight: 800, color: k.color }}>{k.value}</div></div>))}</div>
                  <div style={{ display: "flex", gap: 6, marginBottom: 10 }}><div style={{ background: "#0d1525", borderRadius: 8, padding: "6px 10px", textAlign: "center", borderTop: "2px solid #eab308" }}><div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700 }}>EQUIPES</div><div className="m" style={{ fontSize: 16, fontWeight: 800, color: "#eab308" }}>{eqsAtivas}</div></div>{Object.entries(tipoCount).sort((a, b) => b[1] - a[1]).map(([tipo, count]) => (<div key={tipo} style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "6px 8px", textAlign: "center", borderLeft: "3px solid " + (TIPO_COLORS[tipo] || "#5a7aa0") }}><div style={{ fontSize: 11, fontWeight: 800, color: TIPO_COLORS[tipo] || "#5a7aa0" }}>{tipo}</div><div className="m" style={{ fontSize: 14, fontWeight: 800, color: "#f1f5f9" }}>{count} <span style={{ fontSize: 8, color: "#5a7aa0" }}>eq.</span></div></div>))}</div>
                  <details><summary style={{ fontSize: 10, color: "#3b9eff", cursor: "pointer", fontWeight: 700 }}>▶ Ver/Editar colaboradores ({totalFunc})</summary><div style={{ marginTop: 8, maxHeight: 500, overflowY: "auto" }}>{[...eqEntries,...(admF.length?[["ADM"]]:[]),...(almoxF.length?[["ALMOX"]]:[]),...(patioF.length?[["PATIO"]]:[]),...(treinF.length?[["TREINAMENTO_LV"]]:[])].map(([eq]) => { const eqFuncs = funcs.map((f, i) => ({ ...f, _i: i })).filter(f => f.eq === eq); if (!eqFuncs.length) return null; return (<div key={eq} style={{ marginBottom: 8 }}><div style={{ fontSize: 10, fontWeight: 800, color: ["ADM","ALMOX","PATIO","TREINAMENTO_LV"].includes(eq) ? "#a78bfa" : "#34d399", padding: "4px 0", borderBottom: "1px solid #1a2540" }}>{eq} ({eqFuncs.length})</div>{eqFuncs.map(f => (<div key={f._i} style={{ display: "flex", alignItems: "center", padding: "4px 8px", fontSize: 10, borderBottom: "1px solid #0d1525", gap: 4 }}>{editIdx === f._i ? (<div style={{ flex: 1, display: "flex", gap: 4, alignItems: "center" }}><select defaultValue={f.eq} onChange={e => moverFunc(f._i, e.target.value)} style={{ flex: 1, padding: "3px", background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 4, color: "#eab308", fontSize: 9 }}>{allEqs.map(e => <option key={e} value={e}>{e}</option>)}{!allEqs.includes("TREINAMENTO_LV") && <option value="TREINAMENTO_LV">TREINAMENTO_LV</option>}{!allEqs.includes("ADM") && <option value="ADM">ADM</option>}</select><button onClick={() => setEditIdx(null)} style={{ padding: "2px 6px", background: "#1e2d48", color: "#5a7aa0", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>✕</button></div>) : (<><span style={{ color: "#d4dce9", flex: 1 }}>{f.nome}</span><span style={{ color: "#5a7aa0", fontSize: 9, marginRight: 4 }}>{f.fn}</span><button onClick={() => setEditIdx(f._i)} style={{ padding: "2px 5px", background: "rgba(59,158,255,.1)", color: "#3b9eff", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>↔</button><button onClick={() => removerFunc(f._i)} style={{ padding: "2px 5px", background: "rgba(239,68,68,.1)", color: "#ef4444", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>✕</button></>)}</div>))}</div>); })}</div></details>
                </div>
              );
            })()}
            ...
            {(() => {
              ...
              let prodAnt = 0;
              EQUIPES.filter(eq => eq.meta > 0).forEach(eq => { prodAnt += getEqMonthUS(eq.id, mesAnt); });
              
              // Produção mês atual
              let prodAtual = 0;
              EQUIPES.filter(eq => eq.meta > 0).forEach(eq => { prodAtual += getEqMonthUS(eq.id, histMonth); });
            ...
            {/* Histórico diário */}
            <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 6px", marginBottom: 8 }}>
                <span style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>📊 Produção Diária</span>
                <select value={histEq} onChange={e => setHistEq(e.target.value)} style={{ padding: "4px 6px", background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 6, color: "#94a3b8", fontSize: 10, cursor: "pointer" }}>
                  <option value="all">Todas</option>
                  {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}
                </select>
              </div>
              <div style={{ display: "flex", gap: 12, padding: "0 6px", marginBottom: 6 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 10, borderRadius: 2, background: "rgba(52,211,153,.3)" }}></span>Realizado</span>
                <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 3, borderRadius: 1, background: "#3b9eff" }}></span>Meta</span>
              </div>
              <ResponsiveContainer width="100%" height={200}>
                <ComposedChart data={histData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gradRealGer" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="realUS" name="Realizado" stroke="#34d399" strokeWidth={2.5} fill="url(#gradRealGer)" dot={false} />
      ...
      {screen === "detalhe" && selEq && (() => {
        ...
        return (
          <div style={{ padding: "14px 14px 100px" }}>
            ...
            {(() => { const v = VEICULOS[eq?.nome?.replace(" ","")]; return v?.placa ? (
              <div style={{ display: "flex", gap: 6, marginBottom: 12, overflowX: "auto" }}>
                <div style={{ flex: "0 0 auto", padding: "4px 10px", background: "rgba(59,158,255,.08)", borderRadius: 6, border: "1px solid rgba(59,158,255,.15)", fontSize: 9, color: "#3b9eff" }}>🚛 {v.placa}</div>
                {v.marca && <div style={{ flex: "0 0 auto", padding: "4px 10px", background: "rgba(52,211,153,.08)", borderRadius: 6, border: "1px solid rgba(52,211,153,.15)", fontSize: 9, color: "#34d399" }}>{v.marca}</div>}
                {v.guindaste && <div style={{ flex: "0 0 auto", padding: "4px 10px", background: "rgba(234,179,8,.08)", borderRadius: 6, border: "1px solid rgba(234,179,8,.15)", fontSize: 9, color: "#eab308" }}>🏗️ {v.guindaste}</div>}
              </div>
            ) : null; })()}
            ...
            {allA.map(atrib => {
              ...
              return (
                <div key={aKey} style={{ background: "#111d33", borderRadius: 12, padding: 12, marginBottom: 10, border: "1px solid #1e2d48" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#60a5fa", marginBottom: 2, wordBreak: "break-all" }}>{nota?.nome}</div>
                  {(() => { const ni = getNotaInfo(nota?.nome); return (ni.municipio || ni.prazo) ? <div style={{ fontSize: 9, color: "#5a7aa0", marginBottom: 6 }}>📍 {ni.municipio}{ni.prazo ? " · ⏰ " + ni.prazo : ""}</div> : <div style={{ marginBottom: 6 }} />; })()}
                  {pts.map(pt => { const s = st[pt.id] || "pending"; const ptCava = cavas.find(c => c.atribId === aKey && c.pontoId === pt.id); const prepEq = ptCava ? EQUIPES.find(e => e.id === ptCava.prepEqId) : null;
                    ...
                    <div key={pt.id}>
                      <div onClick={() => !isGestor && togglePonto(aKey, pt.id)} style={{ background: s === "ok" ? "rgba(34,197,94,.06)" : s === "no" ? "rgba(239,68,68,.06)" : "#0b1121", borderRadius: (ptCava || (s === "ok" && pt.svcs?.length)) ? "8px 8px 0 0" : "8px", padding: "8px 10px", marginBottom: (ptCava || (s === "ok" && pt.svcs?.length)) ? 0 : 3, border: "1px solid " + (s === "ok" ? "rgba(34,197,94,.15)" : s === "no" ? "rgba(239,68,68,.15)" : "#1a2540"), display: "flex", alignItems: "center", gap: 8, cursor: isGestor ? "default" : "pointer" }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                          <span className="m" style={{ fontSize: 10, fontWeight: 700, color: s === "ok" ? "#34d399" : s === "no" ? "#ef4444" : "#eab308" }}>{fUS(ptCava ? ptRealU - getCavaValue(ptCava.prepEqId) : ptRealU)} US</span>
                          ...
                          {pt.svcs.map(sv => {
                            ...
                            return (
                              <div key={sv.id} onClick={(e) => {
                                ...
                                fbSet(bp+`/atribs/${aKey}/svcStatus`, newSvcSt);
                      ...
                      {ptCava && (<div style={{ background: "rgba(249,115,22,.06)", borderRadius: "0 0 8px 8px", padding: "4px 10px", marginBottom: 3, border: "1px solid rgba(249,115,22,.12)", borderTop: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 9, color: "#f97316" }}>🔧 Cava: -{getCavaValue(ptCava.prepEqId)} US → {prepEq?.nome} - {prepEq?.enc}</span>
                        {!isGestor && <button onClick={e => { e.stopPropagation(); fbRemove(bp+`/cavas/${ptCava._fbKey}`); }} style={{ background: "none", border: "none", color: "#4b6080", cursor: "pointer", fontSize: 10 }}>✕</button>}
                      </div>)}
                    </div>
                  ); })}
      ...
      {/* ═══ QUALIDADE ═══ */}
      {screen === "qualidade" && !atrasoForm && !retForm && (() => {
        const [qualView, setQualView] = [retNotaBusca?.startsWith("retrab_") ? "retrabalhos" : "atrasos", (v) => setRetNotaBusca(v === "retrabalhos" ? "retrab_view" : "")];
        
        // ── RETRABALHOS VIEW ──
        if (qualView === "retrabalhos") {
          const [retFilterEq, setRetFilterEq] = [histEq, setHistEq];
          const rAll = retrab.filter(r => r.data?.startsWith(histMonth));
          const rM = retFilterEq !== "all" ? rAll.filter(r => r.eqId === retFilterEq) : rAll;
          const totalRetrab = rM.reduce((s, r) => s + (Number(r.qtd) || 1), 0);
          const eqsAfetadas = new Set(rM.map(r => r.eqId)).size;
          const motivoCount = {};
          rM.forEach(r => { motivoCount[r.motivo || "Outro"] = (motivoCount[r.motivo || "Outro"] || 0) + 1; });
          const motivoEntries = Object.entries(motivoCount).sort((a, b) => b[1] - a[1]);
          const eR = EQUIPES.filter(eq => eq.meta > 0).map(eq => {
            const eqR = rAll.filter(r => r.eqId === eq.id);
            return { ...eq, retrab: eqR.reduce((s, r) => s + (Number(r.qtd) || 1), 0) };
          }).filter(e => e.retrab > 0).sort((a, b) => b.retrab - a.retrab);
          const RC = ["#ef4444","#f97316","#facc15","#3b9eff","#a78bfa","#34d399","#f472b6","#22d3ee"];
          
          return (
            <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 12 }}>📊 Qualidade</h2>
              <div style={{ display: "flex", gap: 0, marginBottom: 12, borderRadius: 10, overflow: "hidden", border: "1px solid #1e2d48" }}>
                <button onClick={() => setQualView("atrasos")} style={{ flex: 1, padding: "10px 0", background: "#111d33", color: "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>⏰ Atrasos</button>
                <button onClick={() => setQualView("retrabalhos")} style={{ flex: 1, padding: "10px 0", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🔄 Retrabalhos</button>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <MonthNav value={histMonth} onChange={setHistMonth} style={{ flex: 1 }} />
                <button onClick={() => setRetForm({ eqId: "", data: dataSel, motivos: [], pontos: [] })} style={{ marginLeft: 8, padding: "7px 14px", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Lançar</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
                {[{l:"Total Retrab.",v:totalRetrab,c:"#ef4444"},{l:"Equipes",v:eqsAfetadas,c:"#f97316"},{l:"Motivos",v:Object.keys(motivoCount).length,c:"#facc15"}].map(k => (
                  <div key={k.l} style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid " + k.c }}>
                    <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.l}</div>
                    <div className="m" style={{ fontSize: 20, fontWeight: 800, color: k.c }}>{k.v}</div>
                  </div>
                ))}
              </div>
              {motivoEntries.length > 0 && (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 4, marginBottom: 6 }}>Por Motivo</div>
                  <ResponsiveContainer width="100%" height={180}>
                    <PieChart><Pie data={motivoEntries.map(([n, v]) => ({ name: n, value: v }))} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" stroke="none">
                      {motivoEntries.map((_, i) => <Cell key={i} fill={RC[i % RC.length]} />)}
                    </Pie><Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 11, color: "#f1f5f9" }}>{payload[0]?.name}: {payload[0]?.value}</div></div> : null} /></PieChart>
                  </ResponsiveContainer>
                  <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "4px 10px" }}>
                    {motivoEntries.map(([n, c], i) => <span key={n} style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 8, color: "#94a3b8" }}><span style={{ width: 7, height: 7, borderRadius: 2, background: RC[i % RC.length] }}></span>{n} ({c})</span>)}
                  </div>
                </div>
              )}
              {eR.length > 0 && (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 4, marginBottom: 6 }}>Ranking por Equipe</div>
                  {eR.slice(0, 8).map((eq, i) => (
                    <div key={eq.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px", marginBottom: 2 }}>
                      <span style={{ fontSize: 10, fontWeight: 800, color: ["#eab308","#94a3b8","#cd7f32"][i] || "#5a7aa0", width: 16 }}>{i + 1}º</span>
                      <span style={{ flex: 1, fontSize: 10, color: "#d4dce9" }}>{eq.nome} - {eq.enc}</span>
                      <span className="m" style={{ fontSize: 11, fontWeight: 800, color: "#ef4444" }}>{eq.retrab}</span>
                    </div>
                  ))}
                </div>
              )}
              <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Registros ({rM.length})</div>
              <select value={retFilterEq} onChange={e => setRetFilterEq(e.target.value)} style={{ ...inp, marginBottom: 8, fontSize: 11 }}><option value="all">Todas equipes</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}</select>
              {rM.sort((a, b) => (b.data || "").localeCompare(a.data || "")).map((r, i) => {
                const eq = EQUIPES.find(e => e.id === r.eqId);
                return (
                  <div key={r._fbKey || i} style={{ background: "#111d33", borderRadius: 8, padding: "8px 10px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid #ef4444" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <div><div style={{ fontSize: 10, fontWeight: 700, color: "#e2e8f0" }}>{r.pontoNome} · {eq?.nome || r.eqId}</div><div style={{ fontSize: 9, color: "#5a7aa0" }}>{r.data?.split("-").reverse().join("/")} · Nota: {r.notaId}</div></div>
                      <span style={{ fontSize: 9, fontWeight: 700, color: "#ef4444" }}>{r.qtd || 1}x</span>
                    </div>
                    <div style={{ fontSize: 9, color: "#f97316", marginTop: 2 }}>{r.motivo}</div>
                    {r.obs && <div style={{ fontSize: 8, color: "#5a7aa0", marginTop: 1 }}>{r.obs}</div>}
                  </div>
                );
              })}
              {rM.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "#3a5070", fontSize: 11 }}>Nenhum retrabalho neste mês</div>}
            </div>
          );
        }
        
        // ── ATRASOS VIEW ──
        const aM = atrasos.filter(a => a.data?.startsWith(histMonth));
        const motivoCount = {};
        aM.forEach(a => { motivoCount[a.motivo || "Outro"] = (motivoCount[a.motivo || "Outro"] || 0) + 1; });
        const motivoEntries = Object.entries(motivoCount).sort((a, b) => b[1] - a[1]);
        const eqCount = {};
        aM.forEach(a => { const eq = a.equipe || "Não informado"; eqCount[eq] = (eqCount[eq] || 0) + 1; });
        const eqEntries = Object.entries(eqCount).sort((a, b) => b[1] - a[1]);
        const MOTIVO_COLORS = ["#ef4444", "#f97316", "#facc15", "#3b9eff", "#a78bfa"];
        
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9" }}>📊 Qualidade</h2>
              <button onClick={() => setAtrasoForm({ nota: "", equipe: "", data: dataSel, motivo: "", descricao: "", horasAtraso: "", minutosAtraso: "" })} style={{ padding: "7px 14px", background: "linear-gradient(135deg,#f97316,#ea580c)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Atraso</button>
            </div>
            <div style={{ display: "flex", gap: 0, marginBottom: 12, borderRadius: 10, overflow: "hidden", border: "1px solid #1e2d48" }}>
              <button onClick={() => setQualView("atrasos")} style={{ flex: 1, padding: "10px 0", background: "linear-gradient(135deg,#f97316,#ea580c)", color: "#fff", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>⏰ Atrasos</button>
              <button onClick={() => setQualView("retrabalhos")} style={{ flex: 1, padding: "10px 0", background: "#111d33", color: "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🔄 Retrabalhos</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #f97316" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Atrasos</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#f97316" }}>{aM.length}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #ef4444" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Equipes Afetadas</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#ef4444" }}>{Object.keys(eqCount).length}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #facc15" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Motivos</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#facc15" }}>{Object.keys(motivoCount).length}</div>
              </div>
            </div>
            ...
            {motivoEntries.length > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 4, marginBottom: 6 }}>Por Motivo</div>
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart><Pie data={motivoEntries.map(([name, value]) => ({ name, value }))} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" stroke="none">
                    {motivoEntries.map((_, i) => <Cell key={i} fill={MOTIVO_COLORS[i % MOTIVO_COLORS.length]} />)}
                  </Pie><Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 11, color: "#f1f5f9" }}>{payload[0]?.name}</div><div style={{ fontSize: 12, fontWeight: 700, color: "#f97316" }}>{payload[0]?.value} atraso(s)</div></div> : null} /></PieChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "4px 12px" }}>
                  {motivoEntries.map(([name, count], i) => <span key={name} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 8, height: 8, borderRadius: 2, background: MOTIVO_COLORS[i % MOTIVO_COLORS.length] }}></span>{name} ({count})</span>)}
                </div>
              </div>
            )}
            
            {/* Ranking equipes com mais atrasos */}
            {eqEntries.length > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 4, marginBottom: 6 }}>Atrasos por Equipe</div>
                <ResponsiveContainer width="100%" height={Math.max(120, eqEntries.length * 30)}>
                  <BarChart data={eqEntries.map(([nome, count]) => ({ nome, count }))} layout="vertical" margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e2d48" horizontal={false} />
                    <XAxis type="number" tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} />
                    <YAxis type="category" dataKey="nome" tick={{ fill: "#94a3b8", fontSize: 9 }} axisLine={false} tickLine={false} width={80} />
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 11, color: "#f1f5f9" }}>{payload[0]?.payload?.nome}</div><div style={{ fontSize: 12, fontWeight: 700, color: "#f97316" }}>{payload[0]?.value} atraso(s)</div></div> : null} />
            ...
            {/* Lista de atrasos */}
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Registros ({aM.length})</div>
            {aM.sort((a, b) => (b.data).localeCompare(a.data)).map((a, i) => (
              <div key={a._fbKey || i} style={{ background: "#111d33", borderRadius: 10, padding: "10px 12px", marginBottom: 4, border: "1px solid #1e2d48", borderLeft: "3px solid #f97316" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>Nota: {a.nota}</div>
                    <div style={{ fontSize: 9, color: "#5a7aa0" }}>{a.equipe} · {a.data?.split("-").reverse().join("/")}{a.horasAtraso || a.minutosAtraso ? " · " + (a.horasAtraso || "0") + "h" + (a.minutosAtraso || "00") + "min" : ""}</div>
                  </div>
                  <span style={{ fontSize: 9, fontWeight: 700, color: "#f97316", background: "rgba(249,115,22,.1)", padding: "2px 8px", borderRadius: 6 }}>{a.motivo}</span>
                </div>
                {a.descricao && <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 4 }}>{a.descricao}</div>}
              </div>
            ))}
            {aM.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "#3a5070", fontSize: 11 }}>Nenhum atraso registrado neste mês</div>}
          </div>
        );
      })()}
      
      {/* ATRASO FORM */}
      {screen === "qualidade" && atrasoForm && (() => {
        const MOTIVOS_ATRASO = ["Problema de acesso ao local", "Condições climáticas", "Problema técnico", "Falta de equipe/pessoal", "Falta de material", "Retrabalho", "Pendência do cliente/concessionária", "Reprogramação pela contratante", "Outro"];
        const f = atrasoForm; const setF = (k, v) => setAtrasoForm(p => ({ ...p, [k]: v }));
        const salvar = () => {
          if (!f.nota || !f.motivo) { alert("Preencha nota e motivo"); return; }
          push(ref(db, bp + "/atrasos"), { ...f, criadoEm: new Date().toISOString() });
          setAtrasoForm(null);
        };
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <button onClick={() => setAtrasoForm(null)} style={bk}>← Voltar</button>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 14 }}>⏰ Registrar Atraso de Obra</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Nota / Obra *</label><input value={f.nota} onChange={e => setF("nota", e.target.value)} placeholder="Número da nota" style={{ ...inp, marginTop: 4 }} /></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Equipe</label><select value={f.equipe} onChange={e => setF("equipe", e.target.value)} style={{ ...inp, marginTop: 4 }}><option value="">Selecione (opcional)</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.nome}>{eq.nome} - {eq.enc}</option>)}</select></div>
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Data *</label><input type="date" value={f.data} onChange={e => setF("data", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div>
                <div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Horas</label><input type="number" value={f.horasAtraso} onChange={e => setF("horasAtraso", e.target.value)} placeholder="0" style={{ ...inp, marginTop: 4 }} /></div><div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Minutos</label><input type="number" value={f.minutosAtraso} onChange={e => setF("minutosAtraso", e.target.value)} placeholder="0" style={{ ...inp, marginTop: 4 }} /></div>
              </div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Motivo do Atraso *</label>
                <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>
                  {MOTIVOS_ATRASO.map(m => (
                    <button key={m} onClick={() => setF("motivo", m)} style={{ padding: "10px 12px", background: f.motivo === m ? "rgba(249,115,22,.1)" : "#1a2540", color: f.motivo === m ? "#f97316" : "#5a7aa0", border: f.motivo === m ? "1.5px solid rgba(249,115,22,.3)" : "1px solid #2d3d56", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", textAlign: "left" }}>⏰ {m}</button>
                  ))}
                </div>
              </div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Descrição / Observação</label><textarea value={f.descricao} onChange={e => setF("descricao", e.target.value)} rows={3} placeholder="Descreva o motivo do atraso..." style={{ ...inp, marginTop: 4, resize: "vertical" }} /></div>
              <button onClick={salvar} style={{ width: "100%", padding: "14px 0", marginTop: 8, background: "linear-gradient(135deg,#f97316,#ea580c)", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 800, cursor: "pointer" }}>⏰ Registrar Atraso</button>
            </div>
          </div>
        );
      })()}
      ...
      {/* ═══ OCORRÊNCIAS ═══ */}
      {screen === "DISABLED_ocorrencias" && false && (() => {
        const oAll = ocorrencias;
        const oM = oAll.filter(o => o.data?.startsWith(histMonth));
        const incidentes = oM.filter(o => o.tipo?.includes("Incidente")).length;
        const acidentes = oM.filter(o => o.tipo?.includes("Acidente")).length;
        const tipoCount = {};
        oM.forEach(o => { tipoCount[o.tipo || "Outro"] = (tipoCount[o.tipo || "Outro"] || 0) + 1; });
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9" }}>🚨 Ocorrências</h2>
              <button onClick={() => setOcorrenciaForm({ responsavel: "", equipe: "", colaborador: "", data: dataSel, hora: "", tipoLocal: "", municipio: "", endereco: "", numero: "", descricao: "", tipo: "" })} style={{ padding: "7px 14px", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Registrar</button>
            </div>
            <MonthNav value={histMonth} onChange={setHistMonth} style={{ width: "100%", marginBottom: 12 }} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[{l:"Total",v:oM.length,c:"#f97316"},{l:"Incidentes",v:incidentes,c:"#f97316"},{l:"Acidentes",v:acidentes,c:"#ef4444"}].map(k => (
                <div key={k.l} style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
                  <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.l}</div>
                  <div className="m" style={{ fontSize: 20, fontWeight: 800, color: k.c }}>{k.v}</div>
                </div>
              ))}
            ...
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginTop: 14, marginBottom: 6 }}>Registros ({oM.length})</div>
            {oM.sort((a, b) => (b.data + b.hora).localeCompare(a.data + a.hora)).map((o, i) => (
              <div key={o._fbKey || i} style={{ background: "#111d33", borderRadius: 10, padding: "10px 12px", marginBottom: 4, border: "1px solid #1e2d48", borderLeft: "3px solid " + (o.tipo?.includes("Acidente") ? "#ef4444" : "#f97316") }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div><div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{o.colaborador}</div><div style={{ fontSize: 9, color: "#5a7aa0" }}>{o.equipe} · {o.data?.split("-").reverse().join("/")} {o.hora}</div></div>
                  <span style={{ fontSize: 9, fontWeight: 700, color: o.tipo?.includes("Acidente") ? "#ef4444" : "#f97316", background: o.tipo?.includes("Acidente") ? "rgba(239,68,68,.1)" : "rgba(249,115,22,.1)", padding: "2px 8px", borderRadius: 6 }}>{o.tipo}</span>
                </div>
                <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 4 }}>{o.descricao?.substring(0, 120)}{o.descricao?.length > 120 ? "..." : ""}</div>
                <div style={{ fontSize: 8, color: "#3a5070", marginTop: 2 }}>{o.municipio}{o.endereco ? " · " + o.endereco : ""}</div>
              </div>
            ))}
            {oM.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "#3a5070", fontSize: 11 }}>Nenhuma ocorrência neste mês</div>}
          </div>
        );
      })()}
      {screen === "seguranca" && ocorrenciaForm && (() => {
        const TIPOS_OC = ["Incidente","Acidente Material","Acidente de Trânsito","Incidente de Alto Potencial","Acidente Pessoal sem Afastamento","Acidente Pessoal com Afastamento"];
        const f = ocorrenciaForm; const setF = (k, v) => setOcorrenciaForm(p => ({ ...p, [k]: v }));
        const salvar = () => { if (!f.equipe||!f.colaborador||!f.descricao||!f.tipo) { alert("Preencha os campos obrigatórios"); return; } push(ref(db, bp+"/ocorrencias"), { ...f, criadoEm: new Date().toISOString() }); setOcorrenciaForm(null); };
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <button onClick={() => setOcorrenciaForm(null)} style={bk}>← Voltar</button>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 14 }}>🚨 Registro de Ocorrência</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Responsável *</label><input value={f.responsavel} onChange={e => setF("responsavel", e.target.value)} placeholder="Nome do responsável" style={{ ...inp, marginTop: 4 }} /></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Equipe *</label><select value={f.equipe} onChange={e => setF("equipe", e.target.value)} style={{ ...inp, marginTop: 4 }}><option value="">Selecione</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.nome}>{eq.nome} - {eq.enc}</option>)}</select></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Colaborador envolvido *</label><input value={f.colaborador} onChange={e => setF("colaborador", e.target.value)} placeholder="Nome completo" style={{ ...inp, marginTop: 4 }} /></div>
              <div style={{ display: "flex", gap: 8 }}><div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Data *</label><input type="date" value={f.data} onChange={e => setF("data", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div><div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Hora *</label><input type="time" value={f.hora} onChange={e => setF("hora", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Local *</label><div style={{ display: "flex", gap: 8, marginTop: 4 }}>{["Via Pública","Depósito/EA"].map(t => <button key={t} onClick={() => setF("tipoLocal", t)} style={{ flex: 1, padding: "10px 0", background: f.tipoLocal === t ? "rgba(59,158,255,.15)" : "#1a2540", color: f.tipoLocal === t ? "#3b9eff" : "#5a7aa0", border: f.tipoLocal === t ? "1.5px solid rgba(59,158,255,.3)" : "1px solid #2d3d56", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{t}</button>)}</div></div>
              <div style={{ display: "flex", gap: 8 }}><div style={{ flex: 2 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Município</label><input value={f.municipio} onChange={e => setF("municipio", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div><div style={{ flex: 2 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Endereço</label><input value={f.endereco} onChange={e => setF("endereco", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div><div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Nº</label><input value={f.numero} onChange={e => setF("numero", e.target.value)} placeholder="s/N" style={{ ...inp, marginTop: 4 }} /></div></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Descrição *</label><textarea value={f.descricao} onChange={e => setF("descricao", e.target.value)} rows={4} placeholder="Descreva o que aconteceu..." style={{ ...inp, marginTop: 4, resize: "vertical" }} /></div>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Tipo da Ocorrência *</label><div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>{TIPOS_OC.map(t => { const isAc = t.startsWith("Acidente"); return <button key={t} onClick={() => setF("tipo", t)} style={{ padding: "10px 12px", background: f.tipo === t ? (isAc ? "rgba(239,68,68,.1)" : "rgba(249,115,22,.1)") : "#1a2540", color: f.tipo === t ? (isAc ? "#ef4444" : "#f97316") : "#5a7aa0", border: f.tipo === t ? "1.5px solid " + (isAc ? "#ef4444" : "#f97316") + "40" : "1px solid #2d3d56", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", textAlign: "left" }}>{isAc ? "🔴" : "🟠"} {t}</button>; })}</div></div>
              <button onClick={salvar} style={{ width: "100%", padding: "14px 0", marginTop: 8, background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 800, cursor: "pointer" }}>🚨 Registrar Ocorrência</button>
            </div>
          </div>
        );
      })()}
      ...
      {/* ═══ PROGRAMAÇÃO GERENTE ═══ */}
      {screen === "programacao_ger" && isGestor && (() => {
        const [ty2, tm2, td2] = today.split("-").map(Number);
        const todayD = new Date(ty2, tm2 - 1, td2); todayD.setHours(12);
        const dow = todayD.getDay();
        const mon = new Date(todayD); mon.setDate(todayD.getDate() - (dow === 0 ? 6 : dow - 1));
        const sun = new Date(mon); sun.setDate(mon.getDate() + 6);
        const monL = new Date(mon); monL.setDate(mon.getDate() - 7);
        const monN = new Date(mon); monN.setDate(mon.getDate() + 7);
        const sunL = new Date(monL); sunL.setDate(monL.getDate() + 6);
        const sunN = new Date(monN); sunN.setDate(monN.getDate() + 6);
        const mesI = new Date(todayD.getFullYear(), todayD.getMonth(), 1);
        const mesF = new Date(todayD.getFullYear(), todayD.getMonth() + 1, 0);
        const fmt = (d) => d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
        const prs = { "hoje": { l: "Hoje", i: today, f: today }, "esta_semana": { l: "Esta Semana", i: fmt(mon), f: fmt(sun) }, "semana_passada": { l: "Sem. Passada", i: fmt(monL), f: fmt(sunL) }, "proxima_semana": { l: "Próx. Semana", i: fmt(monN), f: fmt(sunN) }, "este_mes": { l: "Este Mês", i: fmt(mesI), f: fmt(mesF) }, "personalizado": { l: "Personalizado", i: dataSel, f: dataSel } };
        const [pp, setPP] = [statusFilter, setStatusFilter];
        const [ps, setPS] = [statusSearch, setStatusSearch];
        const pr = prs[pp] || prs["hoje"];
        const pL = (s) => { const [y, m, dd] = s.split("-").map(Number); return new Date(y, m - 1, dd); };
        const dr = []; let dd = pL(pr.i); const dF = pL(pr.f); while (dd <= dF) { dr.push(fmt(dd)); dd.setDate(dd.getDate() + 1); }
        const allProg = progRaw || {};
        let aN = []; dr.forEach(dt => { const dp = allProg[dt]; if (dp) { const nl = Array.isArray(dp.notas) ? dp.notas : Object.values(dp.notas || {}); nl.forEach(n => aN.push({ ...n, dia: dt })); } });
        const fl = ps ? aN.filter(n => [n.nota, n.municipio, n.regulatorio, n.status].join(" ").toLowerCase().includes(ps.toLowerCase())) : aN;
        const tN = fl.length, tU = fl.reduce((s, n) => s + (Number(n.us) || 0), 0), mD = dr.length > 0 ? tU / dr.length : 0;
        const dc = dr.map(dt => { const dn = aN.filter(n => n.dia === dt); return { dia: dt.slice(8), data: dt, us: Math.round(dn.reduce((s, n) => s + (Number(n.us) || 0), 0)), notas: dn.length }; });
        const sc = {}; fl.forEach(n => { const st = n.status || "Sem status"; sc[st] = (sc[st] || 0) + 1; });
        const RC = ["#3b9eff", "#34d399", "#facc15", "#ef4444", "#a78bfa", "#f97316", "#f472b6", "#22d3ee"];
        const rc = {}; fl.forEach(n => { const r = n.regulatorio || "Outros"; rc[r] = (rc[r] || 0) + 1; }); const re = Object.entries(rc).sort((a, b) => b[1] - a[1]);
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 12 }}>📋 Programação — {baseNome}</h2>
            <div style={{ display: "flex", gap: 4, marginBottom: 10, overflowX: "auto" }}>{Object.entries(prs).map(([k, p]) => <button key={k} onClick={() => setPP(k)} style={{ flex: "0 0 auto", padding: "8px 12px", background: pp === k ? "linear-gradient(135deg,#eab308,#d97706)" : "#111d33", color: pp === k ? "#0b1121" : "#5a7aa0", border: pp === k ? "none" : "1px solid #1e2d48", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{p.l}</button>)}</div>
            {pp === "personalizado" && <input type="date" value={dataSel} onChange={e => setDataSel(e.target.value)} style={{ width: "100%", marginBottom: 10, ...inp }} />}
            <div style={{ fontSize: 10, color: "#5a7aa0", marginBottom: 12, textAlign: "center" }}>{pr.i === pr.f ? pr.i.split("-").reverse().join("/") : <span>{pr.i.split("-").reverse().join("/")} → {pr.f.split("-").reverse().join("/")} <strong style={{ color: "#eab308" }}>({dr.length} dias)</strong></span>}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>{[{l:"Notas",v:tN,c:"#eab308"},{l:"Total US",v:Math.round(tU),c:"#34d399"},{l:"Média/dia",v:Math.round(mD),c:"#3b9eff"}].map(k => <div key={k.l} style={{ background: "#111d33", borderRadius: 12, padding: "12px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid " + k.c }}><div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.l}</div><div className="m" style={{ fontSize: 24, fontWeight: 800, color: k.c }}>{k.v}</div></div>)}</div>
            {dc.length > 1 && <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}><div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>US por Dia</div><ResponsiveContainer width="100%" height={180}><ComposedChart data={dc} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="#1e2d48" vertical={false} /><XAxis dataKey="dia" tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={{ stroke: "#1e2d48" }} tickLine={false} /><YAxis tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} /><Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.data?.split("-").reverse().join("/")}</div><div style={{ fontSize: 11, color: "#34d399" }}>{payload[0]?.value} US</div><div style={{ fontSize: 10, color: "#5a7aa0" }}>{payload[0]?.payload?.notas} nota(s)</div></div> : null} /><Bar dataKey="us" radius={[4, 4, 0, 0]} barSize={16}>{dc.map((d, i) => <Cell key={i} fill={d.us > 0 ? "#eab308" : "#1e2d48"} />)}</Bar><Line type="monotone" dataKey={() => Math.round(mD)} stroke="#3b9eff" strokeWidth={2} strokeDasharray="6 4" dot={false} /></ComposedChart></ResponsiveContainer></div>}
            {re.length > 0 && <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}><div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 4, marginBottom: 6 }}>Por Tipo</div><ResponsiveContainer width="100%" height={160}><PieChart><Pie data={re.map(([n, v]) => ({ name: n, value: v }))} cx="50%" cy="50%" innerRadius={35} outerRadius={60} dataKey="value" stroke="none">{re.map((_, i) => <Cell key={i} fill={RC[i % RC.length]} />)}</Pie><Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 11, color: "#f1f5f9" }}>{payload[0]?.name}: {payload[0]?.value}</div></div> : null} /></PieChart></ResponsiveContainer><div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "4px 12px" }}>{re.map(([n, c], i) => <span key={n} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 8, height: 8, borderRadius: 2, background: RC[i % RC.length] }}></span>{n} ({c})</span>)}</div></div>}
            {Object.entries(sc).sort((a, b) => b[1] - a[1]).map(([st, c]) => <div key={st} style={{ background: "#111d33", borderRadius: 6, padding: "6px 10px", marginBottom: 3, border: "1px solid #1e2d48", display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 11, color: "#d4dce9" }}>{st}</span><span className="m" style={{ fontSize: 12, fontWeight: 800, color: st.includes("EXECUÇ") ? "#eab308" : st.includes("REPROG") ? "#f97316" : "#5a7aa0" }}>{c}</span></div>)}
            <input value={ps} onChange={e => setPS(e.target.value)} placeholder="🔍 Pesquisar..." style={{ ...inp, marginTop: 10, marginBottom: 10 }} />
            {tN === 0 && <div style={{ textAlign: "center", padding: 24, color: "#3a5070", fontSize: 12 }}>Nenhuma obra programada</div>}
            {(() => {
              // Agrupar por data
              const porDia = {};
              fl.forEach(n => { const d = n.dia || "sem-data"; if (!porDia[d]) porDia[d] = []; porDia[d].push(n); });
              const dias = Object.keys(porDia).sort();
              return dias.map(dia => {
                const notasDia = porDia[dia];
                const usDia = notasDia.reduce((s, n) => s + (Number(n.us) || 0), 0);
                const diaFmt = dia !== "sem-data" ? dia.split("-").reverse().join("/") : "Sem data";
                const dw = dia !== "sem-data" ? ["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"][new Date(dia.split("-")[0], dia.split("-")[1]-1, dia.split("-")[2]).getDay()] : "";
                return (
                  <div key={dia} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", background: "rgba(234,179,8,.06)", borderRadius: 8, border: "1px solid rgba(234,179,8,.12)", marginBottom: 4 }}>
                      <span style={{ fontSize: 11, fontWeight: 800, color: "#eab308" }}>📅 {diaFmt} {dw && <span style={{ fontWeight: 600, color: "#5a7aa0" }}>({dw})</span>}</span>
                      <span style={{ fontSize: 10, color: "#5a7aa0" }}>{notasDia.length} nota(s) · <span className="m" style={{ color: "#34d399", fontWeight: 800 }}>{Math.round(usDia)} US</span></span>
                    </div>
                    {notasDia.map((n, j) => <div key={j} style={{ background: "#111d33", borderRadius: 8, padding: "8px 10px", marginBottom: 2, border: "1px solid #1e2d48", borderLeft: "3px solid #eab308" }}><div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ flex: 1 }}><div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{n.nota}</div><div style={{ fontSize: 9, color: "#5a7aa0" }}>{n.municipio}{n.regulatorio ? " · " + n.regulatorio : ""}</div>{n.status && <div style={{ fontSize: 8, color: n.status.includes("EXECUÇ") ? "#eab308" : n.status.includes("CONCLU") ? "#34d399" : "#f97316", fontWeight: 600 }}>{n.status}</div>}</div><div style={{ textAlign: "right" }}><div className="m" style={{ fontSize: 13, fontWeight: 800, color: "#34d399" }}>{n.us}</div><div style={{ fontSize: 8, color: "#5a7aa0" }}>US</div></div></div></div>)}
                  </div>
                );
              });
            })()}
      ...
      {screen === "DISABLED_retrabalho_old" && false && (() => {
        const [retFilterEq, setRetFilterEq] = [histEq, setHistEq];
        const rAll = retrab.filter(r => r.data?.startsWith(histMonth));
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {rM.length > 0 && (() => {
              const [retSearch, setRetSearch] = [retNotaBusca, setRetNotaBusca];
              return (
                <div style={{ marginTop: 14 }}>
                  ...
                  <input value={retSearch} onChange={e => setRetSearch(e.target.value)} placeholder="🔍 Pesquisar por nota, equipe, motivo..." style={{ ...inp, marginBottom: 8, fontSize: 11 }} />
                  {(() => {
                    const searchTerm = (retSearch || "").toLowerCase();
      ...
      {screen === "qualidade" && retForm && (() => {
        ...
        return (
          <div style={{ padding: "14px 14px 100px" }}>
            ...
            <FL label="Equipe"><select value={retForm.eqId} onChange={e => setRetForm(f => ({ ...f, eqId: e.target.value }))} style={inp}><option value="">Selecione</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}<option value="supervisor">Supervisor</option></select></FL>
      ...
      {/* ═══ INSPEÇÕES (gestor) ═══ */}
      {screen === "seguranca" && !inspecaoForm && !ocorrenciaForm && (() => {
        const [secView, setSecView] = [statusSearch?.startsWith("ocorr_") ? "ocorrencias" : "inspecoes", (v) => setStatusSearch(v === "ocorrencias" ? "ocorr_view" : "")];
        
        // ── OCORRÊNCIAS VIEW ──
        if (secView === "ocorrencias") {
          const oAll = ocorrencias;
          const oM = oAll.filter(o => o.data?.startsWith(histMonth));
          const incidentes = oM.filter(o => o.tipo?.includes("Incidente")).length;
          const acidentes = oM.filter(o => o.tipo?.includes("Acidente")).length;
          ...
          return (
            <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 12 }}>🔒 Segurança</h2>
              <div style={{ display: "flex", gap: 0, marginBottom: 12, borderRadius: 10, overflow: "hidden", border: "1px solid #1e2d48" }}>
                <button onClick={() => setSecView("inspecoes")} style={{ flex: 1, padding: "10px 0", background: "#111d33", color: "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🛡️ Inspeções</button>
                <button onClick={() => setSecView("ocorrencias")} style={{ flex: 1, padding: "10px 0", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🚨 Ocorrências</button>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <MonthNav value={histMonth} onChange={setHistMonth} style={{ flex: 1 }} />
                <button onClick={() => setOcorrenciaForm({ responsavel: "", equipe: "", colaborador: "", data: dataSel, hora: "", tipoLocal: "", municipio: "", endereco: "", numero: "", descricao: "", tipo: "" })} style={{ marginLeft: 8, padding: "7px 14px", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Registrar</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
                {[{l:"Total",v:oM.length,c:"#f97316"},{l:"Incidentes",v:incidentes,c:"#f97316"},{l:"Acidentes",v:acidentes,c:"#ef4444"}].map(k => (
                  <div key={k.l} style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
        ...
        // ── INSPEÇÕES VIEW ──
        const inspFilterEq = histEq;
        const isFiltered = inspFilterEq !== "all";
        
        // If filtered by team, show ALL months; if not, filter by month
        const iAll = isFiltered ? inspecoes.filter(i => i.eqId === inspFilterEq) : inspecoes;
        const iM = isFiltered ? iAll : iAll.filter(i => i.data?.startsWith(histMonth));
        ...
        const eqInsp = EQUIPES.filter(eq => eq.meta > 0).map(eq => {
        ...
        // Chart data
        const chartData = eqInsp.filter(e => e.total > 0).map(e => ({ nome: e.enc, comportamento: e.avgComp, condicao: e.avgCond }));
        const conformes = totalInsp - pontuadas;
        const donutData = [{ name: "100% Conforme", value: conformes }, { name: "Pontuadas", value: pontuadas }];
        const DONUT_COLS = ["#34d399", "#ef4444"];
        
        // Historical chart for filtered team (by month)
        const histChartData = isFiltered ? (() => {
          const months = {};
          iAll.forEach(i => { const m = (i.data || "").slice(0, 7); if (m) { if (!months[m]) months[m] = { comp: [], cond: [] }; months[m].comp.push(Number(i.comportamento) || 0); months[m].cond.push(Number(i.condicao) || 0); } });
          return Object.entries(months).sort((a, b) => a[0].localeCompare(b[0])).map(([m, d]) => ({
            mes: MESES[parseInt(m.split("-")[1])] || m, mesKey: m,
            comportamento: Math.round(d.comp.reduce((s, v) => s + v, 0) / d.comp.length * 10) / 10,
            condicao: Math.round(d.cond.reduce((s, v) => s + v, 0) / d.cond.length * 10) / 10,
            inspecoes: d.comp.length,
          }));
        })() : [];
        
        const filteredEq = isFiltered ? EQUIPES.find(e => e.id === inspFilterEq) : null;

        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9" }}>🔒 Segurança</h2>
              <button onClick={() => setInspecaoForm({ eqId: "", data: dataSel, comportamento: "", condicao: "", obs: "" })} style={{ padding: "7px 14px", background: "linear-gradient(135deg,#3b9eff,#2563eb)", color: "#fff", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>+ Inspeção</button>
            </div>
            <div style={{ display: "flex", gap: 0, marginBottom: 12, borderRadius: 10, overflow: "hidden", border: "1px solid #1e2d48" }}>
              <button onClick={() => setSecView("inspecoes")} style={{ flex: 1, padding: "10px 0", background: "linear-gradient(135deg,#3b9eff,#2563eb)", color: "#fff", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🛡️ Inspeções</button>
              <button onClick={() => setSecView("ocorrencias")} style={{ flex: 1, padding: "10px 0", background: "#111d33", color: "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🚨 Ocorrências</button>
            </div>
            
            {/* Filters */}
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              {!isFiltered && <MonthNav value={histMonth} onChange={setHistMonth} style={{ flex: 1 }} />}
              <select value={inspFilterEq} onChange={e => setHistEq(e.target.value)} style={{ flex: 1, ...inp }}>
                <option value="all">Todas equipes</option>
                {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}
              </select>
            </div>
            ...
            {/* Team history header */}
            {isFiltered && filteredEq && (
              <div style={{ background: "linear-gradient(135deg,#0d1829,#162240)", borderRadius: 12, padding: "12px 14px", marginBottom: 12, border: "1px solid #1a2d4d" }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: "#f1f5f9" }}>{filteredEq.nome} - {filteredEq.enc}</div>
                <div style={{ fontSize: 10, color: "#5a7aa0" }}>Histórico completo · {iAll.length} inspeção(ões) · {pontuadas} pontuação(ões)</div>
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              ...
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{isFiltered ? "Taxa Conf." : "Equipes insp."}</div>
                <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#34d399" }}>{isFiltered ? (totalInsp > 0 ? Math.round(conformes / totalInsp * 100) : 0) + "%" : eqInsp.filter(e => e.total > 0).length + "/" + EQUIPES.filter(eq => eq.meta > 0).length}</div>
              </div>
            </div>
            ...
            {/* Historical chart for filtered team */}
            {isFiltered && histChartData.length > 1 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>Evolução Mensal — {filteredEq?.enc}</div>
                <ResponsiveContainer width="100%" height={200}>
                  <ComposedChart data={histChartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e2d48" vertical={false} />
                    <XAxis dataKey="mes" tick={{ fill: "#5a7aa0", fontSize: 9 }} axisLine={{ stroke: "#1e2d48" }} tickLine={false} />
                    <YAxis domain={[0, 100]} tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} tickFormatter={v => v + "%"} />
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 12, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.mesKey}</div><div style={{ fontSize: 11, color: "#3b9eff" }}>Comportamento: {payload[0]?.value}%</div><div style={{ fontSize: 11, color: "#a78bfa" }}>Condição: {payload[1]?.value}%</div><div style={{ fontSize: 10, color: "#5a7aa0" }}>{payload[0]?.payload?.inspecoes} inspeção(ões)</div></div> : null} />
                    <Line type="monotone" dataKey="comportamento" stroke="#3b9eff" strokeWidth={2.5} dot={{ r: 4, fill: "#3b9eff" }} />
                    <Line type="monotone" dataKey="condicao" stroke="#a78bfa" strokeWidth={2.5} dot={{ r: 4, fill: "#a78bfa" }} />
                    <ReferenceLine y={100} stroke="#34d399" strokeDasharray="4 4" strokeWidth={1.5} />
                  </ComposedChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 4 }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 3, borderRadius: 2, background: "#3b9eff" }}></span>Comportamento</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 3, borderRadius: 2, background: "#a78bfa" }}></span>Condição</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 10, height: 2, borderRadius: 1, background: "#34d399" }}></span>Meta 100%</span>
                </div>
              </div>
            )}
      ...
      {screen === "seguranca" && inspecaoForm && (
        <div style={{ padding: "14px 14px 100px" }}>
          ...
          <FL label="Equipe"><select value={inspecaoForm.eqId} onChange={e => setInspecaoForm(f => ({ ...f, eqId: e.target.value }))} style={inp}><option value="">Selecione</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eq.nome} - {eq.enc}</option>)}</select></FL>
      ...
      {screen === "status" && (() => {
        ...
        // Notas do dia selecionado (atribuídas nesse dia)
        ...
        // Notas do mês: TODAS as notas importadas (não só as atribuídas)
        const notasFiltered = notas;
        ...
        // Current list based on mode + status filter
        const listaNotasRaw = statusMode === "dia" ? notasDoDia : filteredMes;
        const listaNotas = (statusFilter && statusFilter !== "all" && statusFilter !== "dia_mode")
          ? listaNotasRaw.filter(n => getStatus(n.id) === statusFilter)
          : listaNotasRaw;
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 12, paddingBottom: 4 }}>
              {STATUS_OPTS.map(s => {
                const isActive = statusFilter === s.value || (statusFilter === "dia_mode" && false);
                const thisCount = counts[s.value] || 0;
                return (
                <button key={s.value || "none"} onClick={() => setStatusFilter(statusFilter === s.value ? (statusMode === "dia" ? "dia_mode" : "all") : s.value)} style={{ flex: "0 0 auto", padding: "8px 12px", background: statusFilter === s.value ? s.bg || "rgba(59,158,255,.08)" : "#111d33", border: statusFilter === s.value ? "2px solid " + s.color : "1px solid #1e2d48", borderRadius: 8, textAlign: "center", minWidth: 65, cursor: "pointer" }}>
                  <div style={{ fontSize: 14 }}>{s.icon}</div>
                  <div className="m" style={{ fontSize: 16, fontWeight: 800, color: s.color }}>{thisCount}</div>
            ...
            {statusFilter !== "all" && statusFilter !== "dia_mode" && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, padding: "6px 10px", background: "rgba(59,158,255,.06)", borderRadius: 8, border: "1px solid rgba(59,158,255,.15)" }}>
                <span style={{ fontSize: 11, color: "#94a3b8" }}>Filtro: <strong style={{ color: STATUS_OPTS.find(s => s.value === statusFilter)?.color }}>{STATUS_OPTS.find(s => s.value === statusFilter)?.label || "Sem status"}</strong></span>
                <button onClick={() => setStatusFilter(statusMode === "dia" ? "dia_mode" : "all")} style={{ fontSize: 10, color: "#3b9eff", background: "none", border: "none", cursor: "pointer", fontWeight: 700 }}>✕ Limpar</button>
              </div>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {listaNotas.map(n => {
                ...
                const nInfo = getNotaInfo(n.nome);
                return (
                  <div key={n.id} style={{ background: st.bg || "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid " + (st.color || "#1e2d48") + "20" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {(nInfo.municipio || nInfo.prazo) && <div style={{ fontSize: 9, color: "#3b9eff" }}>📍 {nInfo.municipio}{nInfo.prazo ? " · ⏰ " + nInfo.prazo : ""}</div>}
                      </div>
                    ...
                    {/* Execution tracking button for partial/cancelled/prep */}
                    {(() => {
                      const status = getStatus(n.id);
                      const notaAtribs = atribs.filter(a => a.notaId === n.id);
                      const diasUnicos = new Set(notaAtribs.map(a => a.eqId + "_" + a.data)).size;
                      if (status === "exec_parcial" || status === "cancelada" || diasUnicos > 1) {
                        return <button onClick={() => setNotaDetalhe(n)} style={{ marginTop: 4, width: "100%", padding: "6px 0", background: "rgba(234,179,8,.08)", border: "1px solid rgba(234,179,8,.15)", borderRadius: 6, color: "#eab308", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>📊 Execução ({diasUnicos} ida{diasUnicos !== 1 ? "s" : ""})</button>;
                      }
                      return null;
                    })()}
                  </div>
      ...
      {/* ═══ DETALHE EXECUÇÃO NOTA ═══ */}
      {screen === "status" && notaDetalhe && (() => {
        const n = notaDetalhe;
        const nInfo = getNotaInfo(n.nome);
        const usTotal = n.u || 0;
        
        // Buscar todas atribuições desta nota (vindas do lançamento de pontos)
        const notaAtribs = atribs.filter(a => a.notaId === n.id).sort((a, b) => (a.data || "").localeCompare(b.data || ""));
        
        // Agrupar por equipe+dia (cada atribuição = 1 ida)
        const idasMap = {};
        notaAtribs.forEach(a => {
          const eq = EQUIPES.find(e => e.id === a.eqId);
          const key = a.eqId + "_" + a.data;
          if (!idasMap[key]) {
            idasMap[key] = { eqId: a.eqId, equipe: eq?.nome || a.eqId, enc: eq?.enc || "", meta: eq?.meta || 25, data: a.data, pontos: 0, us: 0 };
          }
          const pts = (a.pIds || []).length;
          const ptsUS = getPts(a.notaId, a.pIds || []).reduce((s, p) => s + (p.u || 0), 0);
          idasMap[key].pontos += pts;
          idasMap[key].us += ptsUS;
        });
        
        const idasList = Object.values(idasMap).sort((a, b) => a.data.localeCompare(b.data));
        const totalIdas = idasList.length;
        const eqsUsadas = [...new Set(idasList.map(i => i.eqId))];
        
        // US gasto = soma das metas de cada equipe/dia
        let usGasto = 0;
        const usPorEq = {};
        idasList.forEach(ida => {
          usGasto += ida.meta;
          if (!usPorEq[ida.equipe]) usPorEq[ida.equipe] = { idas: 0, us: 0, meta: ida.meta };
          usPorEq[ida.equipe].idas += 1;
          usPorEq[ida.equipe].us += ida.meta;
        });
        const usRestante = Math.max(0, usTotal - usGasto);
        const pctGasto = usTotal > 0 ? Math.min(100, Math.round(usGasto / usTotal * 100)) : 0;
        const chartData = Object.entries(usPorEq).map(([eq, d]) => ({ equipe: eq.replace("SRO ", ""), idas: d.idas, us: Math.round(d.us) }));
        
        const adicionarIda = () => {
          if (!idaForm?.equipe) { alert("Selecione a equipe"); return; }
          push(ref(db, bp + "/idas"), { notaId: n.id, notaNome: n.nome, equipe: idaForm.equipe, data: idaForm.data, obs: idaForm.obs || "", criadoEm: new Date().toISOString() });
          setIdaForm({ equipe: "", data: dataSel, obs: "" });
        };
        
        const removerIda = (fbKey) => {
          if (!confirm("Remover esta ida?")) return;
          set(ref(db, bp + "/idas/" + fbKey), null);
        };
        
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <button onClick={() => { setNotaDetalhe(null); setIdaForm(null); }} style={bk}>← Voltar</button>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 4 }}>📊 Controle de Execução</h2>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#60a5fa", marginBottom: 2 }}>{n.nome}</div>
            {(nInfo.municipio || nInfo.prazo) && <div style={{ fontSize: 10, color: "#5a7aa0", marginBottom: 10 }}>📍 {nInfo.municipio}{nInfo.prazo ? " · ⏰ " + nInfo.prazo : ""}</div>}
            
            {/* KPIs */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 6px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #eab308" }}>
                <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Idas</div>
                <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#eab308" }}>{totalIdas}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 6px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #34d399" }}>
                <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>US Total</div>
                <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#34d399" }}>{fUS(usTotal)}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 6px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #ef4444" }}>
                <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>US Gasto</div>
                <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#ef4444" }}>{fUS(usGasto)}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 6px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid " + (usRestante > 0 ? "#3b9eff" : "#34d399") }}>
                <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Restante</div>
                <div className="m" style={{ fontSize: 20, fontWeight: 800, color: usRestante > 0 ? "#3b9eff" : "#34d399" }}>{fUS(usRestante)}</div>
              </div>
            </div>
            
            {/* Progress bar */}
            <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid #1e2d48", marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700 }}>Progresso de execução</span>
                <span className="m" style={{ fontSize: 11, fontWeight: 800, color: pctGasto >= 100 ? "#ef4444" : pctGasto >= 75 ? "#facc15" : "#34d399" }}>{pctGasto}%</span>
              </div>
              <div style={{ height: 8, background: "rgba(255,255,255,.05)", borderRadius: 4, overflow: "hidden" }}>
                <div style={{ height: "100%", width: Math.min(pctGasto, 100) + "%", background: pctGasto >= 100 ? "linear-gradient(90deg,#ef4444,#dc2626)" : pctGasto >= 75 ? "linear-gradient(90deg,#facc15,#eab308)" : "linear-gradient(90deg,#34d399,#22c55e)", borderRadius: 4, transition: "width .5s" }} />
              </div>
              <div style={{ fontSize: 8, color: "#5a7aa0", marginTop: 4, textAlign: "center" }}>{fUS(usGasto)} de {fUS(usTotal)} US consumidos · {eqsUsadas.length} equipe(s)</div>
            </div>
            
            {/* Chart: idas por equipe */}
            {chartData.length > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>US Gasto por Equipe</div>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e2d48" vertical={false} />
                    <XAxis dataKey="equipe" tick={{ fill: "#94a3b8", fontSize: 9 }} axisLine={{ stroke: "#1e2d48" }} tickLine={false} />
                    <YAxis tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} />
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.equipe}</div><div style={{ fontSize: 11, color: "#ef4444" }}>{payload[0]?.payload?.us} US gastos</div><div style={{ fontSize: 10, color: "#5a7aa0" }}>{payload[0]?.payload?.idas} ida(s)</div></div> : null} />
                    <Bar dataKey="us" radius={[4, 4, 0, 0]} barSize={28}>
                      {chartData.map((_, i) => <Cell key={i} fill={["#ef4444","#f97316","#facc15","#3b9eff","#a78bfa","#34d399"][i % 6]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div style={{ background: "rgba(59,158,255,.06)", borderRadius: 8, padding: "8px 12px", border: "1px solid rgba(59,158,255,.15)", marginBottom: 14, fontSize: 9, color: "#3b9eff" }}>
              ℹ️ Os dados são calculados automaticamente a partir dos <strong>lançamentos de pontos</strong> no Painel. Cada atribuição de equipe a esta nota conta como uma ida.
            </div>
            
            {/* Idas list from atribs */}
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Histórico ({totalIdas} idas)</div>
            {idasList.map((ida, i) => (
              <div key={i} style={{ background: "#111d33", borderRadius: 8, padding: "8px 12px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid " + ["#eab308","#3b9eff","#34d399","#a78bfa","#f97316"][i % 5], display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{ida.equipe} - {ida.enc}</div>
                  <div style={{ fontSize: 9, color: "#5a7aa0" }}>{ida.data?.split("-").reverse().join("/")} · {ida.pontos} pts executados</div>
                </div>
                <span className="m" style={{ fontSize: 11, fontWeight: 800, color: "#ef4444" }}>-{fUS(ida.meta)} US</span>
              </div>
            ))}
            {totalIdas === 0 && <div style={{ textAlign: "center", padding: 20, color: "#3a5070", fontSize: 11 }}>Nenhuma ida registrada</div>}
          </div>
        );
      })()}
        ...
        <div style={{ padding: "14px 14px 100px" }}>
          ...
          <FL label="Equipe de Preparação">
            <select value={prepForm.eqId} onChange={e => setPrepForm(f => ({ ...f, eqId: e.target.value }))} style={inp}>
              ...
              {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eqLabel(eq)} ({eq.tipo})</option>)}
            </select>
          </FL>
          ...
          <FL label="Quantidade de cavas">
            ...
            <div className="m" style={{ fontSize: 12, color: "#f97316", fontWeight: 700, marginTop: 6 }}>
              = {fUS(prepForm.qtdCavas * getCavaValue(prepForm.eqId))} US ({prepForm.qtdCavas} × {getCavaValue(prepForm.eqId)} US)
            </div>
          </FL>
          ...
          <button onClick={() => {
            ...
            fbPush(bp+"/preps", {
              ...
              us: prepForm.qtdCavas * getCavaValue(prepForm.eqId),
              obs: prepForm.obs || "",
            });
            setPrepForm(null);
            setScreen("home");
      ...
      {screen === "preparacao_view" && (
        <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
          ...
          {(() => {
            ...
            // Per equipe
            const eqPreps = EQUIPES.filter(eq => eq.meta > 0).map(eq => ({
              ...eq,
      ...
      {screen === "atribuir" && atribForm && (
        <div style={{ padding: "14px 14px 100px" }}>
          ...
          <FL label="Equipe"><select value={atribForm.eqId} onChange={e => setAtribForm(f => ({ ...f, eqId: e.target.value }))} style={inp}><option value="">Selecione</option>{EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.id}>{eqLabel(eq)} ({eq.tipo})</option>)}</select></FL>
          <FL label="Nota / Obra">
      ...
      {cavaModal && (() => {
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "#111d33", borderRadius: 16, padding: 20, maxWidth: 340, width: "100%", border: "1px solid #1e2d48", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ fontSize: 12, color: "#5a7aa0", marginBottom: 12 }}>Ponto <strong style={{ color: "#eab308" }}>{cavaModal.pontoNome}</strong> — houve cava?</div>
            ...
            <div style={{ maxHeight: 180, overflowY: "auto", display: "flex", flexDirection: "column", gap: 3, marginBottom: 12 }}>
              {EQUIPES.filter(eq => eq.meta > 0).map(eq => (<button key={eq.id} onClick={() => salvarCava(eq.id)} style={{ padding: "9px 12px", background: "#0b1121", border: "1px solid #1e2d48", borderRadius: 8, cursor: "pointer", textAlign: "left", fontSize: 11, fontWeight: 600, color: "#d4dce9", display: "flex", justifyContent: "space-between" }}><span>{eq.nome} - {eq.enc}</span><span style={{ color: tipoCor(eq.tipo), fontSize: 9 }}>{eq.tipo}</span></button>))}
            </div>
