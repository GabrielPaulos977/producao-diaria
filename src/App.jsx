...
export default function App() {
  const [role, setRole] = useState(null); // null | "auxiliar" | "gestor" | "coordenador" 
  ...
  // Funcionarios: use Firebase if available, seed from static data on first use
  const funcionarios = useMemo(() => {
    if (funcRaw === null) return FUNCIONARIOS[baseSel] || [];
  ...
  // Seed Firebase with static data if empty
  useEffect(() => {
    if (bp && funcRaw !== null && funcRaw !== undefined && Object.keys(funcRaw || {}).length === 0) {
      const staticFuncs = FUNCIONARIOS[baseSel] || [];
      if (staticFuncs.length > 0) fbSet(bp + "/funcionarios", staticFuncs);
    }
  }, [bp, funcRaw, baseSel]);
  ...
  // ═══ LOGIN SCREEN ═══
  if (!role || (!baseSel && role !== "coordenador")) {
    return (
  ...
  const isProgramador = false;
  const tabs = isCoordenador
    ? [{ k: "visao_geral", l: "Visão Geral" }, { k: "relatorio", l: "Relatório" }, { k: "programacao_dia", l: "Programação" }]
    : isGestor
    ? [{ k: "home", l: "Painel" }, { k: "gerencial", l: "Gerencial" }, { k: "ranking", l: "Ranking" }, { k: "programacao_ger", l: "Programação" }, { k: "retrabalho", l: "Retrab." }, { k: "preparacao_view", l: "Prepar." }, { k: "inspecoes", l: "Inspeções" }, { k: "status", l: "Status" }, { k: "import", l: "Importar" }]
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      ...
      {screen === "gerencial" && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* Comparativo Mensal */}
            {(() => {
              // Employee data from Firebase (or static fallback)
              ...
              // All unique team names for move dropdown
              const allEqs = [...new Set(funcs.map(f => f.eq))].sort();
              
              // Management state (using retNotaBusca for toggle, histEq for editing)
              const [editingFunc, setEditingFunc] = [retForm?.editFunc || null, (v) => setRetForm(f => f ? { ...f, editFunc: v } : { editFunc: v })];
              const [addingFunc, setAddingFunc] = [retForm?.addFunc || false, (v) => setRetForm(f => f ? { ...f, addFunc: v } : { addFunc: v })];
              const [newFunc, setNewFunc] = [retForm?.newFunc || { eq: "", nome: "", fn: "", re: "" }, (v) => setRetForm(f => f ? { ...f, newFunc: v } : { newFunc: v })];
              
              const saveFunc = (updatedList) => { fbSet(bp + "/funcionarios", updatedList); };
              
              const moveFunc = (idx, newEq) => {
                const updated = [...funcs];
                updated[idx] = { ...updated[idx], eq: newEq };
                saveFunc(updated);
              };
              
              const removeFunc = (idx) => {
                if (!confirm("Remover " + funcs[idx].nome + "?")) return;
                const updated = funcs.filter((_, i) => i !== idx);
                saveFunc(updated);
              };
              
              const addFunc = () => {
                if (!newFunc.nome || !newFunc.eq) return;
                saveFunc([...funcs, { ...newFunc }]);
                setNewFunc({ eq: "", nome: "", fn: "", re: "" });
                setAddingFunc(false);
              };
              ...
              return (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 14px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    ...
                    <button onClick={() => setAddingFunc(!addingFunc)} style={{ padding: "4px 10px", background: addingFunc ? "#ef4444" : "#34d399", color: addingFunc ? "#fff" : "#0b1121", border: "none", borderRadius: 6, fontSize: 9, fontWeight: 700, cursor: "pointer" }}>{addingFunc ? "✕ Cancelar" : "+ Adicionar"}</button>
                  </div>
                  
                  {/* Add new employee form */}
                  {addingFunc && (
                    <div style={{ background: "#0d1525", borderRadius: 8, padding: 10, marginBottom: 10, border: "1px solid #1a2d4d" }}>
                      <select value={newFunc.eq} onChange={e => setNewFunc({ ...newFunc, eq: e.target.value })} style={{ ...inp, marginBottom: 4, fontSize: 11 }}>
                        <option value="">Selecione a equipe</option>
                        {allEqs.map(eq => <option key={eq} value={eq}>{eq}</option>)}
                        {EQUIPES.filter(eq => eq.meta > 0).map(eq => !allEqs.includes(eq.nome.replace(" ","")) && <option key={eq.id} value={eq.nome.replace(" ","")}>{eq.nome}</option>)}
                        <option value="ADM">ADM</option><option value="ALMOX">ALMOX</option><option value="PATIO">PATIO</option><option value="TREINAMENTO_LV">Treinamento LV</option>
                      </select>
                      <input value={newFunc.nome} onChange={e => setNewFunc({ ...newFunc, nome: e.target.value })} placeholder="Nome completo" style={{ ...inp, marginBottom: 4, fontSize: 11 }} />
                      <input value={newFunc.fn} onChange={e => setNewFunc({ ...newFunc, fn: e.target.value })} placeholder="Função" style={{ ...inp, marginBottom: 4, fontSize: 11 }} />
                      <input value={newFunc.re} onChange={e => setNewFunc({ ...newFunc, re: e.target.value })} placeholder="RE (opcional)" style={{ ...inp, marginBottom: 6, fontSize: 11 }} />
                      <button onClick={addFunc} style={{ width: "100%", padding: "8px 0", background: "#34d399", color: "#0b1121", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Adicionar Colaborador</button>
                    </div>
                  )}
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr 1fr", gap: 5, marginBottom: 10 }}>
                    {[
                      ...
                      { label: "Trein.LV", value: treinF.length, color: "#f97316" },
                  ...
                  <details>
                    <summary style={{ fontSize: 10, color: "#3b9eff", cursor: "pointer", fontWeight: 700, marginBottom: 6 }}>▶ Ver/Editar colaboradores ({totalFunc})</summary>
                    <div style={{ marginTop: 8, maxHeight: 500, overflowY: "auto" }}>
                      {[...eqEntries, ...(admF.length ? [["ADM", admF.length]] : []), ...(almoxF.length ? [["ALMOX", almoxF.length]] : []), ...(patioF.length ? [["PATIO", patioF.length]] : []), ...(treinF.length ? [["TREINAMENTO_LV", treinF.length]] : [])].map(([eq]) => {
                        const eqFuncs = funcs.map((f, i) => ({ ...f, _idx: i })).filter(f => f.eq === eq);
                        if (!eqFuncs.length) return null;
                        return (
                          <div key={eq} style={{ marginBottom: 8 }}>
                            <div style={{ fontSize: 10, fontWeight: 800, color: ["ADM","ALMOX","PATIO","TREINAMENTO_LV"].includes(eq) ? "#a78bfa" : "#34d399", padding: "4px 0", borderBottom: "1px solid #1a2540" }}>{eq} ({eqFuncs.length})</div>
                            {eqFuncs.map(f => (
                              <div key={f._idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 8px", fontSize: 10, borderBottom: "1px solid #0d1525" }}>
                                {editingFunc === f._idx ? (
                                  <div style={{ flex: 1, display: "flex", gap: 4, alignItems: "center" }}>
                                    <select value={f.eq} onChange={e => { moveFunc(f._idx, e.target.value); setEditingFunc(null); }} style={{ padding: "3px 4px", background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 4, color: "#eab308", fontSize: 9, flex: 1 }}>
                                      {allEqs.map(e => <option key={e} value={e}>{e}</option>)}{!allEqs.includes("TREINAMENTO_LV") && <option value="TREINAMENTO_LV">TREINAMENTO_LV</option>}{!allEqs.includes("ADM") && <option value="ADM">ADM</option>}{!allEqs.includes("ALMOX") && <option value="ALMOX">ALMOX</option>}{!allEqs.includes("PATIO") && <option value="PATIO">PATIO</option>}
                                    </select>
                                    <button onClick={() => setEditingFunc(null)} style={{ padding: "2px 6px", background: "#1e2d48", color: "#5a7aa0", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>✕</button>
                                  </div>
                                ) : (
                                  <>
                                    <span style={{ color: "#d4dce9", flex: 1 }}>{f.nome}</span>
                                    <span style={{ color: "#5a7aa0", fontSize: 9, marginRight: 6 }}>{f.fn}</span>
                                    <button onClick={() => setEditingFunc(f._idx)} title="Mover" style={{ padding: "2px 5px", background: "rgba(59,158,255,.1)", color: "#3b9eff", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer", marginRight: 2 }}>↔</button>
                                    <button onClick={() => removeFunc(f._idx)} title="Remover" style={{ padding: "2px 5px", background: "rgba(239,68,68,.1)", color: "#ef4444", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>✕</button>
                                  </>
                                )}
                              </div>
                            ))}
      ...
      {/* ═══ OCORRÊNCIAS (dentro de Inspeções) ═══ */}
      {screen === "inspecoes" && ocorrenciaForm && (() => {
        const TIPOS_OCORRENCIA = ["Incidente", "Acidente Material", "Acidente de Trânsito", "Incidente de Alto Potencial", "Acidente Pessoal sem Afastamento", "Acidente Pessoal com Afastamento"];
        const TIPOS_LOCAL = ["Via Pública", "Depósito/EA"];
        const f = ocorrenciaForm;
        const setF = (k, v) => setOcorrenciaForm(prev => ({ ...prev, [k]: v }));
        
        const salvarOcorrencia = () => {
          if (!f.equipe || !f.colaborador || !f.descricao || !f.tipo) { alert("Preencha todos os campos obrigatórios"); return; }
          fbPush(bp + "/ocorrencias", { ...f, criadoEm: new Date().toISOString() });
          setOcorrenciaForm(null);
        };
        
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            <button onClick={() => setOcorrenciaForm(null)} style={{ background: "none", border: "none", color: "#60a5fa", fontSize: 13, fontWeight: 600, cursor: "pointer", marginBottom: 12, padding: 0 }}>← Voltar</button>
            <h2 style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9", marginBottom: 14 }}>🚨 Registro de Ocorrência</h2>
            
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Responsável pelo registro *</label>
              <input value={f.responsavel} onChange={e => setF("responsavel", e.target.value)} placeholder="Nome do responsável" style={{ ...inp, marginTop: 4 }} /></div>
              
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Equipe *</label>
              <select value={f.equipe} onChange={e => setF("equipe", e.target.value)} style={{ ...inp, marginTop: 4 }}>
                <option value="">Selecione</option>
                {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.nome}>{eq.nome} - {eq.enc}</option>)}
              </select></div>
              
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Colaborador envolvido/acidentado *</label>
              <input value={f.colaborador} onChange={e => setF("colaborador", e.target.value)} placeholder="Nome completo" style={{ ...inp, marginTop: 4 }} /></div>
              
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Data *</label>
                <input type="date" value={f.data} onChange={e => setF("data", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div>
                <div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Hora *</label>
                <input type="time" value={f.hora} onChange={e => setF("hora", e.target.value)} style={{ ...inp, marginTop: 4 }} /></div>
              </div>
              
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Tipo do Local *</label>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                {TIPOS_LOCAL.map(t => (
                  <button key={t} onClick={() => setF("tipoLocal", t)} style={{ flex: 1, padding: "10px 0", background: f.tipoLocal === t ? "rgba(59,158,255,.15)" : "#1a2540", color: f.tipoLocal === t ? "#3b9eff" : "#5a7aa0", border: f.tipoLocal === t ? "1.5px solid rgba(59,158,255,.3)" : "1px solid #2d3d56", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{t}</button>
                ))}
              </div></div>
              
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 2 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Município *</label>
                <input value={f.municipio} onChange={e => setF("municipio", e.target.value)} placeholder="Município" style={{ ...inp, marginTop: 4 }} /></div>
                <div style={{ flex: 2 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Endereço</label>
                <input value={f.endereco} onChange={e => setF("endereco", e.target.value)} placeholder="Rua/Av" style={{ ...inp, marginTop: 4 }} /></div>
                <div style={{ flex: 1 }}><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Nº</label>
                <input value={f.numero} onChange={e => setF("numero", e.target.value)} placeholder="s/N" style={{ ...inp, marginTop: 4 }} /></div>
              </div>
              
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Descrição da Ocorrência *</label>
              <textarea value={f.descricao} onChange={e => setF("descricao", e.target.value)} rows={4} placeholder="Descreva o que aconteceu..." style={{ ...inp, marginTop: 4, resize: "vertical" }} /></div>
              
              <div><label style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700 }}>Tipo da Ocorrência *</label>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 4 }}>
                {TIPOS_OCORRENCIA.map(t => {
                  const isAcidente = t.startsWith("Acidente");
                  const color = isAcidente ? "#ef4444" : "#f97316";
                  return (
                    <button key={t} onClick={() => setF("tipo", t)} style={{ padding: "10px 12px", background: f.tipo === t ? (isAcidente ? "rgba(239,68,68,.1)" : "rgba(249,115,22,.1)") : "#1a2540", color: f.tipo === t ? color : "#5a7aa0", border: f.tipo === t ? "1.5px solid " + color + "40" : "1px solid #2d3d56", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", textAlign: "left" }}>{isAcidente ? "🔴" : "🟠"} {t}</button>
                  );
                })}
              </div></div>
              
              <button onClick={salvarOcorrencia} style={{ width: "100%", padding: "14px 0", marginTop: 8, background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 12, fontSize: 14, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 16px rgba(239,68,68,.25)" }}>🚨 Registrar Ocorrência</button>
            </div>
          </div>
        );
      })()}

      {/* ═══ PROGRAMAÇÃO (gerente - base única) ═══ */}
      {screen === "programacao_ger" && isGestor && (() => {
        ...
        const [ty, tm, td] = today.split("-").map(Number);
        const todayDate = new Date(ty, tm - 1, td); todayDate.setHours(12, 0, 0, 0);
        const dayOfWeek = todayDate.getDay();
        const mondayThis = new Date(todayDate); mondayThis.setDate(todayDate.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1));
        const sundayThis = new Date(mondayThis); sundayThis.setDate(mondayThis.getDate() + 6);
        const mondayLast = new Date(mondayThis); mondayLast.setDate(mondayThis.getDate() - 7);
        ...
        // Only current base data
        const allProg = progRaw || {};
        const progData = Object.keys(allProg).length > 0 ? allProg : {};
        
        let allNotas = [];
        datasRange.forEach(dt => {
          const dayProg = progData[dt];
          if (dayProg) {
            const notasList = Array.isArray(dayProg.notas) ? dayProg.notas : Object.values(dayProg.notas || {});
            notasList.forEach(n => allNotas.push({ ...n, dia: dt }));
          }
        ...
        const totalUS = filtered.reduce((s, n) => s + (Number(n.us) || 0), 0);
        const mediaUSDia = datasRange.length > 0 ? totalUS / datasRange.length : 0;
        
        // Daily chart data
        const dailyChartData = datasRange.map(dt => {
          const dayNotas = allNotas.filter(n => n.dia === dt);
          return { dia: dt.slice(8), data: dt, us: Math.round(dayNotas.reduce((s, n) => s + (Number(n.us) || 0), 0)), notas: dayNotas.length };
        });
        
        const statusCount = {};
        filtered.forEach(n => { const st = n.status || "Sem status"; statusCount[st] = (statusCount[st] || 0) + 1; });
        const statusEntries = Object.entries(statusCount).sort((a, b) => b[1] - a[1]);
        
        const regCount = {};
        filtered.forEach(n => { const r = n.regulatorio || "Outros"; regCount[r] = (regCount[r] || 0) + 1; });
        const regEntries = Object.entries(regCount).sort((a, b) => b[1] - a[1]);
        const REG_COLORS = ["#3b9eff", "#34d399", "#facc15", "#ef4444", "#a78bfa", "#f97316", "#f472b6", "#22d3ee"];
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {progPeriodo === "personalizado" && <input type="date" value={dataSel} onChange={e => setDataSel(e.target.value)} style={{ width: "100%", marginBottom: 10, ...inp }} />}
            
            <div style={{ fontSize: 10, color: "#5a7aa0", marginBottom: 12, textAlign: "center" }}>
              {dataInicio === dataFim ? dataInicio.split("-").reverse().join("/") : <span>{dataInicio.split("-").reverse().join("/")} → {dataFim.split("-").reverse().join("/")} <strong style={{ color: "#eab308" }}>({datasRange.length} dias)</strong></span>}
            </div>
              ...
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <ComposedChart data={dailyChartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    ...
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.data?.split("-").reverse().join("/")}</div><div style={{ fontSize: 11, color: "#34d399" }}>{payload[0]?.value} US</div><div style={{ fontSize: 10, color: "#5a7aa0" }}>{payload[0]?.payload?.notas} nota(s)</div></div> : null} />
                    <Bar dataKey="us" radius={[4, 4, 0, 0]} barSize={16}>
                      {dailyChartData.map((dd, i) => <Cell key={i} fill={dd.us > 0 ? "#eab308" : "#1e2d48"} />)}
                    </Bar>
                    <Line type="monotone" dataKey={() => Math.round(mediaUSDia)} stroke="#3b9eff" strokeWidth={2} strokeDasharray="6 4" dot={false} />
                  </ComposedChart>
                </ResponsiveContainer>
            ...
            {regEntries.length > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 8px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart><Pie data={regEntries.map(([name, value]) => ({ name, value }))} cx="50%" cy="50%" innerRadius={35} outerRadius={60} dataKey="value" stroke="none">
                    {regEntries.map((_, i) => <Cell key={i} fill={REG_COLORS[i % REG_COLORS.length]} />)}
                  </Pie><Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "6px 10px" }}><div style={{ fontSize: 11, color: "#f1f5f9" }}>{payload[0]?.name}</div><div style={{ fontSize: 12, fontWeight: 700, color: "#34d399" }}>{payload[0]?.value} nota(s)</div></div> : null} /></PieChart>
                </ResponsiveContainer>
                <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "4px 12px" }}>
                  {regEntries.map(([name, count], i) => <span key={name} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: "#94a3b8" }}><span style={{ width: 8, height: 8, borderRadius: 2, background: REG_COLORS[i % REG_COLORS.length], flexShrink: 0 }}></span>{name} ({count})</span>)}
                </div>
              </div>
            )}
            
            {statusEntries.length > 0 && statusEntries.map(([st, count]) => (
              <div key={st} style={{ background: "#111d33", borderRadius: 6, padding: "6px 10px", marginBottom: 3, border: "1px solid #1e2d48", display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#d4dce9" }}>{st}</span>
                <span className="m" style={{ fontSize: 12, fontWeight: 800, color: st.includes("EXECUÇ") ? "#eab308" : st.includes("REPROG") ? "#f97316" : st.includes("CONCLU") ? "#34d399" : "#5a7aa0" }}>{count}</span>
              </div>
            ))}
            
            <input value={progSearch} onChange={e => setProgSearch(e.target.value)} placeholder="🔍 Pesquisar nota, município, tipo..." style={{ ...inp, marginTop: 10, marginBottom: 10 }} />
            
            {totalNotas === 0 && <div style={{ textAlign: "center", padding: 24, color: "#3a5070", fontSize: 12 }}>Nenhuma obra programada para este período</div>}
            
            {filtered.map((n, j) => (
              <div key={j} style={{ background: "#111d33", borderRadius: 8, padding: "8px 10px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid #eab308" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{n.nota}</div>
                    <div style={{ fontSize: 9, color: "#5a7aa0" }}>{n.municipio}{n.regulatorio ? " · " + n.regulatorio : ""}</div>
                    {n.status && <div style={{ fontSize: 8, color: n.status.includes("EXECUÇ") ? "#eab308" : n.status.includes("REPROG") ? "#f97316" : "#5a7aa0", fontWeight: 600 }}>{n.status}</div>}
                  </div>
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div className="m" style={{ fontSize: 13, fontWeight: 800, color: "#34d399" }}>{n.us}</div>
      ...
      {screen === "inspecoes" && !inspecaoForm && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* Toggle Inspeções / Ocorrências */}
            <div style={{ display: "flex", gap: 0, marginBottom: 12, borderRadius: 10, overflow: "hidden", border: "1px solid #1e2d48" }}>
              <button onClick={() => setStatusSearch("")} style={{ flex: 1, padding: "10px 0", background: !statusSearch?.startsWith("ocorr_") ? "linear-gradient(135deg,#3b9eff,#2563eb)" : "#111d33", color: !statusSearch?.startsWith("ocorr_") ? "#fff" : "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🛡️ Inspeções</button>
              <button onClick={() => setStatusSearch("ocorr_view")} style={{ flex: 1, padding: "10px 0", background: statusSearch?.startsWith("ocorr_") ? "linear-gradient(135deg,#ef4444,#dc2626)" : "#111d33", color: statusSearch?.startsWith("ocorr_") ? "#fff" : "#5a7aa0", border: "none", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>🚨 Ocorrências</button>
            </div>
            
            {/* Ocorrências View */}
            {statusSearch?.startsWith("ocorr_") ? (() => {
              ...
              return (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
                    <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Total</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#f97316" }}>{oM.length}</div>
                    </div>
                    <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Incidentes</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#f97316" }}>{incidentes}</div>
                    </div>
                    <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Acidentes</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#ef4444" }}>{acidentes}</div>
                    </div>
                  </div>
                  ...
                  <button onClick={() => setOcorrenciaForm({ responsavel: "", equipe: "", colaborador: "", data: dataSel, hora: "", tipoLocal: "", municipio: "", endereco: "", numero: "", descricao: "", tipo: "" })} style={{ width: "100%", padding: "12px 0", marginTop: 12, marginBottom: 14, background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 16px rgba(239,68,68,.25)" }}>🚨 Registrar Nova Ocorrência</button>
                  
                  <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Registros ({oM.length})</div>
                    ...
                    <div key={o._fbKey || i} style={{ background: "#111d33", borderRadius: 10, padding: "10px 12px", marginBottom: 4, border: "1px solid #1e2d48", borderLeft: "3px solid " + (o.tipo?.includes("Acidente") ? "#ef4444" : "#f97316") }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <div>
                          ...
                          <div style={{ fontSize: 9, color: "#5a7aa0" }}>{o.equipe} · {o.data?.split("-").reverse().join("/")} {o.hora}</div>
                        </div>
                      ...
                      <div style={{ fontSize: 8, color: "#3a5070", marginTop: 2 }}>{o.municipio}{o.endereco ? " · " + o.endereco : ""} · {o.tipoLocal}</div>
                    </div>
                  ))}
                  {oM.length === 0 && <div style={{ textAlign: "center", padding: 20, color: "#3a5070", fontSize: 11 }}>Nenhuma ocorrência registrada neste mês</div>}
                </>
              );
            })() : (
            <>
