  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      ...
      {screen === "programacao_ger" && isGestor && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {fl.map((n, j) => <div key={j} style={{ background: "#111d33", borderRadius: 8, padding: "8px 10px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid #eab308" }}><div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ flex: 1 }}><div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{n.nota}</div><div style={{ fontSize: 9, color: "#5a7aa0" }}>{n.municipio}{n.regulatorio ? " · " + n.regulatorio : ""}</div>{n.status && <div style={{ fontSize: 8, color: n.status.includes("EXECUÇ") ? "#eab308" : "#f97316", fontWeight: 600 }}>{n.status}</div>}</div><div style={{ textAlign: "right" }}><div className="m" style={{ fontSize: 13, fontWeight: 800, color: "#34d399" }}>{n.us}</div><div style={{ fontSize: 8, color: "#5a7aa0" }}>US</div></div></div></div>)}
          </div>
        );
      })()}
                ...
                return (
                  <div key={n.id} style={{ background: st.bg || "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid " + (st.color || "#1e2d48") + "20" }}>
                    ...
                    {(() => {
                      ...
                      const notaIdas = idas.filter(ida => ida.notaId === n.id);
                      if (status === "exec_parcial" || status === "cancelada" || notaIdas.length > 0) {
                        return <button onClick={() => setNotaDetalhe(n)} style={{ marginTop: 4, width: "100%", padding: "6px 0", background: "rgba(234,179,8,.08)", border: "1px solid rgba(234,179,8,.15)", borderRadius: 6, color: "#eab308", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>📊 Execução ({notaIdas.length} ida{notaIdas.length !== 1 ? "s" : ""})</button>;
                      }
                      return null;
                    })()}
                  </div>
      ...
      {screen === "status" && notaDetalhe && (() => {
        const n = notaDetalhe;
        const nIdas = idas.filter(ida => ida.notaId === n.id).sort((a, b) => (a.data || "").localeCompare(b.data || ""));
        const totalIdas = nIdas.length;
        const eqsUsadas = [...new Set(nIdas.map(i => i.equipe))];
        const nInfo = getNotaInfo(n.nome);
        const usTotal = n.u || 0;
        
        // US gasto = soma das metas diárias de cada equipe por ida
        const getEqMeta = (eqNome) => {
          const eq = EQUIPES.find(e => e.nome === eqNome || eqNome?.includes(e.nome));
          return eq?.meta || 25;
        };
        ...
        nIdas.forEach(ida => {
          const eq = ida.equipe || "?";
          const meta = getEqMeta(eq);
          if (!usPorEq[eq]) usPorEq[eq] = { idas: 0, us: 0, meta };
          usPorEq[eq].idas += 1;
          usPorEq[eq].us += meta;
          usGasto += meta;
        });
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* Form to add ida */}
            <div style={{ background: "#111d33", borderRadius: 12, padding: 12, border: "1px solid #1e2d48", marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 8 }}>+ Registrar Ida</div>
              {!idaForm && <button onClick={() => setIdaForm({ equipe: "", data: dataSel, obs: "" })} style={{ width: "100%", padding: "10px 0", background: "linear-gradient(135deg,#eab308,#d97706)", color: "#0b1121", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 800, cursor: "pointer" }}>+ Nova Ida</button>}
              {idaForm && (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <select value={idaForm.equipe} onChange={e => setIdaForm(f => ({ ...f, equipe: e.target.value }))} style={{ ...inp, fontSize: 11 }}>
                    <option value="">Selecione a equipe</option>
                    {EQUIPES.filter(eq => eq.meta > 0).map(eq => <option key={eq.id} value={eq.nome}>{eq.nome} - {eq.enc}</option>)}
                  </select>
                  <input type="date" value={idaForm.data} onChange={e => setIdaForm(f => ({ ...f, data: e.target.value }))} style={{ ...inp, fontSize: 11 }} />
                  <input value={idaForm.obs} onChange={e => setIdaForm(f => ({ ...f, obs: e.target.value }))} placeholder="Observação (opcional)" style={{ ...inp, fontSize: 11 }} />
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => setIdaForm(null)} style={{ flex: 1, padding: "8px 0", background: "#1e2d48", color: "#5a7aa0", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Cancelar</button>
                    <button onClick={adicionarIda} style={{ flex: 1, padding: "8px 0", background: "#eab308", color: "#0b1121", border: "none", borderRadius: 6, fontSize: 11, fontWeight: 800, cursor: "pointer" }}>Registrar</button>
                  </div>
                </div>
              )}
            </div>
            ...
            {nIdas.map((ida, i) => {
              const metaIda = getEqMeta(ida.equipe);
              return (
              <div key={ida._fbKey || i} style={{ background: "#111d33", borderRadius: 8, padding: "8px 12px", marginBottom: 3, border: "1px solid #1e2d48", borderLeft: "3px solid " + ["#eab308","#3b9eff","#34d399","#a78bfa","#f97316"][i % 5], display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0" }}>{ida.equipe}</div>
                  <div style={{ fontSize: 9, color: "#5a7aa0" }}>{ida.data?.split("-").reverse().join("/")}{ida.obs ? " · " + ida.obs : ""}</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span className="m" style={{ fontSize: 11, fontWeight: 800, color: "#ef4444" }}>-{fUS(metaIda)} US</span>
                  <button onClick={() => removerIda(ida._fbKey)} style={{ padding: "2px 6px", background: "rgba(239,68,68,.1)", color: "#ef4444", border: "none", borderRadius: 4, fontSize: 9, cursor: "pointer" }}>✕</button>
                </div>
              </div>
              );
            })}
