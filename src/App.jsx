  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      ...
      {screen === "status" && (() => {
        ...
        notas.forEach(n => { const st = getStatus(n.id); counts[st] = (counts[st] || 0) + 1; });
        ...
        return (
          <div style={{ padding: "12px 14px 100px" }}>
            ...
            {(() => {
              ...
              // US por status - only notas that have been assigned to teams
              const notasAtribuidas = new Set(atribs.map(a => a.notaId));
              const usPorStatus = {};
              STATUS_OPTS.forEach(s => { usPorStatus[s.value] = 0; });
              notas.filter(n => notasAtribuidas.has(n.id)).forEach(n => { const st = getStatus(n.id); usPorStatus[st] = (usPorStatus[st] || 0) + (n.u || 0); });
              const totalNotasAtrib = notas.filter(n => notasAtribuidas.has(n.id)).length;
            ...
            {/* Notas list */}
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Notas ({filtered.length} de {notas.length})</div>
