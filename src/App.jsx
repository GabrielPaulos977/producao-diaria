...
export default function App() {
  ...
  const bp = baseSel ? `bases/${baseSel}` : null;
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0b1121", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans',system-ui,sans-serif" }}>
        ...
        <div style={{ textAlign: "center", padding: 30, width: "100%", maxWidth: 340 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f8fafc", marginBottom: 4 }}>Indicadores Procel</h1>
          ...
          {!role ? (
            <>
              ...
              {!showSenha ? (
                <button onClick={() => setShowSenha(true)} style={{ width: "100%", padding: "14px 24px", marginBottom: 10, background: "transparent", color: "#60a5fa", border: "2px solid #1e2d48", borderRadius: 12, fontSize: 15, fontWeight: 800, cursor: "pointer" }}>Gestor</button>
              ) : (
              ...
              <button onClick={() => { setRole("coordenador"); setBaseSel("all"); }} style={{ width: "100%", padding: "14px 24px", background: "transparent", color: "#a78bfa", border: "2px solid #1e2d48", borderRadius: 12, fontSize: 15, fontWeight: 800, cursor: "pointer" }}>Coordenador</button>
              <div style={{ fontSize: 10, color: "#4b6080", marginTop: 6 }}>Visão consolidada de todas as bases</div>
            </>
          ) : (
            <>
    ...
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#0f1a2e,#162240)", padding: "10px 16px", borderBottom: "1px solid #1e2d48" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, color: "#f8fafc" }}>Indicadores Procel {baseNome}</div>
              <div style={{ fontSize: 9, color: "#4b6080", fontWeight: 600 }}>
                {role === "coordenador" ? "👑 Coordenador" : isGestor ? "👔 Gestor" : "🔧 Auxiliar"} · {notas.length} notas
              </div>
            </div>
          </div>
      ...
      {screen === "status" && (() => {
        const STATUS_OPTS = [
          ...
          { value: "exec_programado", label: "Executado Programado", color: "#3b9eff", bg: "rgba(59,158,255,.08)", icon: "📋" },
          { value: "exec_parcial", label: "Executado Parcial", color: "#facc15", bg: "rgba(250,204,21,.08)", icon: "⚠️" },
        ];
        ...
        const setStatus = (notaId, value) => fbSet(bp+"/notaStatus/" + notaId, value);

        // Notas that have atribuições in selected month
        const monthAtribs = atribs.filter(a => a.data?.startsWith(histMonth));
        const notasDoMes = new Set(monthAtribs.map(a => a.notaId));
        const notasFiltered = notas.filter(n => notasDoMes.has(n.id));

        // Filter and search
        const filtered = notasFiltered.filter(n => {
        ...
        // Summary counts - only notas from selected month
        const counts = { "": 0, concluida: 0, cancelada: 0, exec_programado: 0, exec_parcial: 0 };
        notasFiltered.forEach(n => { const st = getStatus(n.id); counts[st] = (counts[st] || 0) + 1; });
        ...
        return (
          <div style={{ padding: "12px 14px 100px" }}>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 12, paddingBottom: 4 }}>
              {STATUS_OPTS.map(s => (
                <button key={s.value || "none"} onClick={() => setStatusFilter(statusFilter === s.value ? "all" : s.value)}
                  style={{ flex: "0 0 auto", padding: "8px 12px", background: statusFilter === s.value ? s.bg || "#111d33" : "#111d33", border: statusFilter === s.value ? "1.5px solid " + s.color + "40" : "1px solid #1e2d48", borderRadius: 8, cursor: "pointer", textAlign: "center", minWidth: 70 }}>
                  <div style={{ fontSize: 14 }}>{s.icon}</div>
                  <div className="m" style={{ fontSize: 16, fontWeight: 800, color: s.color }}>{counts[s.value] || 0}</div>
                  <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{s.label || "Sem status"}</div>
                </button>
              ))}
            </div>
            ...
            {statusFilter !== "all" && (
              <div style={{ marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#5a7aa0" }}>Filtro: <strong style={{ color: STATUS_OPTS.find(s => s.value === statusFilter)?.color }}>{STATUS_OPTS.find(s => s.value === statusFilter)?.label}</strong></span>
                <button onClick={() => setStatusFilter("all")} style={{ fontSize: 10, color: "#3b9eff", background: "none", border: "none", cursor: "pointer" }}>Limpar filtro</button>
              </div>
            )}
            ...
            {(() => {
              ...
              const notasAtribuidas = notasDoMes;
              const usPorStatus = {};
              STATUS_OPTS.forEach(s => { usPorStatus[s.value] = 0; });
              notasFiltered.forEach(n => { const st = getStatus(n.id); usPorStatus[st] = (usPorStatus[st] || 0) + (n.u || 0); });
              const totalNotasAtrib = notasFiltered.length;
            ...
            {/* Notas list */}
            <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Notas ({filtered.length} de {notasFiltered.length} no mês)</div>
                  ...
                  <div key={n.id} style={{ background: st.bg || "#111d33", borderRadius: 10, padding: "10px 12px", border: "1px solid " + (st.color || "#1e2d48") + "20" }}>
                    ...
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {STATUS_OPTS.filter(s => s.value).map(s => (
                        <button key={s.value} onClick={() => setStatus(n.id, getStatus(n.id) === s.value ? "" : s.value)}
