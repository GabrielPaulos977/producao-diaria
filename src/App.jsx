  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
      ...
      {screen === "status" && notaDetalhe && (() => {
        ...
        // US por equipe para chart
        const usPorEq = {};
        nIdas.forEach(ida => {
          const eq = ida.equipe || "?";
          usPorEq[eq] = (usPorEq[eq] || 0) + 1;
        });
        const chartData = Object.entries(usPorEq).map(([eq, count]) => ({ equipe: eq.replace("SRO ", ""), idas: count, us: Math.round(usTotal / totalIdas * count) || 0 }));
        const usGasto = totalIdas > 0 ? usTotal : 0;
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* KPIs */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #eab308" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Idas</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#eab308" }}>{totalIdas}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #3b9eff" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>Equipes</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#3b9eff" }}>{eqsUsadas.length}</div>
              </div>
              <div style={{ background: "#111d33", borderRadius: 10, padding: "10px 8px", border: "1px solid #1e2d48", textAlign: "center", borderTop: "3px solid #34d399" }}>
                <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>US Total</div>
                <div className="m" style={{ fontSize: 22, fontWeight: 800, color: "#34d399" }}>{fUS(usTotal)}</div>
              </div>
            </div>
            ...
            {chartData.length > 0 && (
              <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 6px 6px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", paddingLeft: 8, marginBottom: 6 }}>Idas por Equipe</div>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1e2d48" vertical={false} />
                    <XAxis dataKey="equipe" tick={{ fill: "#94a3b8", fontSize: 9 }} axisLine={{ stroke: "#1e2d48" }} tickLine={false} />
                    <YAxis tick={{ fill: "#5a7aa0", fontSize: 8 }} axisLine={false} tickLine={false} allowDecimals={false} />
                    <Tooltip content={({ active, payload }) => active && payload?.length ? <div style={{ background: "#1a2540", border: "1px solid #2d3d56", borderRadius: 8, padding: "8px 12px" }}><div style={{ fontSize: 11, color: "#f1f5f9", fontWeight: 700 }}>{payload[0]?.payload?.equipe}</div><div style={{ fontSize: 11, color: "#eab308" }}>{payload[0]?.value} ida(s)</div></div> : null} />
                    <Bar dataKey="idas" radius={[4, 4, 0, 0]} barSize={28}>
                      {chartData.map((_, i) => <Cell key={i} fill={["#eab308","#3b9eff","#34d399","#a78bfa","#f97316","#ef4444"][i % 6]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
