...
import { BASES, EQUIPES_POR_BASE, INTEGRANTES_POR_TIPO, INIT_NOTAS, DIAS_UTEIS, DIVISOR_US, MOTIVOS_RETRAB, SERVICOS_LISTA } from "./data";
      ...
      {screen === "gerencial" && (() => {
        ...
        return (
          <div className="resp-content" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* Comparativo Mensal */}
            {(() => {
              // Team counts and headcount
              const baseInfo = BASES.find(b => b.id === baseSel);
              const tipoCount = {};
              let totalFunc = 0;
              EQUIPES.filter(eq => eq.meta > 0).forEach(eq => {
                const tipo = eq.tipo || "-";
                if (!tipoCount[tipo]) tipoCount[tipo] = { count: 0, func: 0 };
                tipoCount[tipo].count++;
                tipoCount[tipo].func += INTEGRANTES_POR_TIPO[tipo] || 0;
                totalFunc += INTEGRANTES_POR_TIPO[tipo] || 0;
              });
              const admCount = baseInfo?.adm || 0;
              const totalGeral = totalFunc + admCount;
              const eqsAtivas = EQUIPES.filter(eq => eq.meta > 0).length;
              const TIPO_COLORS = { "B3": "#34d399", "B2": "#3b9eff", "B1": "#facc15", "C1": "#a78bfa", "-": "#5a7aa0" };
              ...
              return (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 14px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  ...
                  {/* Summary */}
                  <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                    <div style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "8px 6px", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0" }}>Equipes Ativas</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#eab308" }}>{eqsAtivas}</div>
                    </div>
                    <div style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "8px 6px", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0" }}>Func. Campo</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#3b9eff" }}>{totalFunc}</div>
                    </div>
                    <div style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "8px 6px", textAlign: "center" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0" }}>ADM</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#a78bfa" }}>{admCount}</div>
                    </div>
                    <div style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "8px 6px", textAlign: "center", borderTop: "2px solid #eab308" }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0" }}>Total</div>
                      <div className="m" style={{ fontSize: 20, fontWeight: 800, color: "#f1f5f9" }}>{totalGeral}</div>
                    </div>
                  </div>
                  
                  {/* Per type */}
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {Object.entries(tipoCount).sort((a, b) => b[1].count - a[1].count).map(([tipo, info]) => (
                      <div key={tipo} style={{ flex: "1 0 auto", minWidth: 70, background: "#0d1525", borderRadius: 8, padding: "6px 10px", borderLeft: "3px solid " + (TIPO_COLORS[tipo] || "#5a7aa0"), textAlign: "center" }}>
                        <div style={{ fontSize: 10, fontWeight: 800, color: TIPO_COLORS[tipo] || "#5a7aa0" }}>{tipo}</div>
                        <div style={{ fontSize: 12, fontWeight: 800, color: "#f1f5f9" }}>{info.count} <span style={{ fontSize: 9, color: "#5a7aa0", fontWeight: 400 }}>eq.</span></div>
                        <div style={{ fontSize: 9, color: "#5a7aa0" }}>{info.func} func.</div>
                      </div>
                    ))}
                  </div>
                </div>
