      ...
      {screen === "gerencial" && (() => {
        ...
        return (
          <div className="resp-content fade-in" style={{ padding: "12px 14px 100px" }}>
            ...
            {/* Comparativo Mensal */}
            {(() => {
              // Employee data from FUNCIONARIOS
              const funcs = (FUNCIONARIOS[baseSel] || []);
              const totalFunc = funcs.length;
              const campo = funcs.filter(f => !["ADM","ALMOX","PATIO"].includes(f.eq));
              const admF = funcs.filter(f => f.eq === "ADM");
              const almoxF = funcs.filter(f => f.eq === "ALMOX");
              const patioF = funcs.filter(f => f.eq === "PATIO");
              
              const fnCount = {};
              funcs.forEach(f => { const fn = f.fn || "Sem função"; fnCount[fn] = (fnCount[fn] || 0) + 1; });
              const fnEntries = Object.entries(fnCount).sort((a, b) => b[1] - a[1]);
              ...
              return (
                <div style={{ background: "#111d33", borderRadius: 14, padding: "12px 14px", border: "1px solid #1e2d48", marginBottom: 14 }}>
                  <div style={{ fontSize: 10, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 10 }}>👥 Efetivo — {baseNome}</div>
                  
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
                    {[
                      { label: "Campo", value: campo.length, color: "#34d399" },
                      { label: "ADM", value: admF.length, color: "#a78bfa" },
                      { label: "Almox", value: almoxF.length, color: "#facc15" },
                      { label: "Pátio", value: patioF.length, color: "#3b9eff" },
                      { label: "Total", value: totalFunc, color: "#f1f5f9" },
                    ].map(k => (
                      <div key={k.label} style={{ background: "#0d1525", borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
                        <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase" }}>{k.label}</div>
                        <div className="m" style={{ fontSize: 16, fontWeight: 800, color: k.color }}>{k.value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                    <div style={{ background: "#0d1525", borderRadius: 8, padding: "6px 10px", textAlign: "center", borderTop: "2px solid #eab308" }}>
                      <div style={{ fontSize: 7, color: "#5a7aa0", fontWeight: 700 }}>EQUIPES</div>
                      <div className="m" style={{ fontSize: 16, fontWeight: 800, color: "#eab308" }}>{eqsAtivas}</div>
                    </div>
                    {Object.entries(tipoCount).sort((a, b) => b[1] - a[1]).map(([tipo, count]) => (
                      <div key={tipo} style={{ flex: 1, background: "#0d1525", borderRadius: 8, padding: "6px 8px", textAlign: "center", borderLeft: "3px solid " + (TIPO_COLORS[tipo] || "#5a7aa0") }}>
                        <div style={{ fontSize: 11, fontWeight: 800, color: TIPO_COLORS[tipo] || "#5a7aa0" }}>{tipo}</div>
                        <div className="m" style={{ fontSize: 14, fontWeight: 800, color: "#f1f5f9" }}>{count} <span style={{ fontSize: 8, color: "#5a7aa0" }}>eq.</span></div>
                      </div>
                    ))}
                  </div>
                  ...
                  {eqEntries.length > 0 && (
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 10 }}>
                      {eqEntries.map(([eq, count]) => (
                        <div key={eq} style={{ background: "#0d1525", borderRadius: 6, padding: "4px 8px", borderLeft: "3px solid #34d399" }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: "#d4dce9" }}>{eq}</span>
                          <span style={{ fontSize: 10, color: "#5a7aa0" }}> ({count})</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {fnEntries.length > 0 && (
                    <div style={{ marginBottom: 8 }}>
                      <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>Por Função</div>
                      {fnEntries.slice(0, 6).map(([fn, count]) => (
                        <div key={fn} style={{ display: "flex", justifyContent: "space-between", padding: "3px 8px", background: "#0d1525", borderRadius: 4, marginBottom: 2 }}>
                          <span style={{ fontSize: 10, color: "#94a3b8" }}>{fn || "Sem função"}</span>
                          <span className="m" style={{ fontSize: 10, fontWeight: 700, color: "#eab308" }}>{count}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <details style={{ marginTop: 4 }}>
                    <summary style={{ fontSize: 10, color: "#3b9eff", cursor: "pointer", fontWeight: 700 }}>▶ Ver lista completa ({totalFunc} colaboradores)</summary>
                    <div style={{ marginTop: 8, maxHeight: 400, overflowY: "auto" }}>
                      {[...eqEntries, ...(admF.length ? [["ADM", admF.length]] : []), ...(almoxF.length ? [["ALMOX", almoxF.length]] : []), ...(patioF.length ? [["PATIO", patioF.length]] : [])].map(([eq]) => {
                        const eqFuncs = funcs.filter(f => f.eq === eq);
                        if (!eqFuncs.length) return null;
                        return (
                          <div key={eq} style={{ marginBottom: 8 }}>
                            <div style={{ fontSize: 10, fontWeight: 800, color: ["ADM","ALMOX","PATIO"].includes(eq) ? "#a78bfa" : "#34d399", padding: "4px 0", borderBottom: "1px solid #1a2540" }}>{eq} ({eqFuncs.length})</div>
                            {eqFuncs.map((f, i) => (
                              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 8px", fontSize: 10 }}>
                                <span style={{ color: "#d4dce9" }}>{f.nome}</span>
                                <span style={{ color: "#5a7aa0", fontSize: 9 }}>{f.fn}</span>
                              </div>
                            ))}
                          </div>
                        );
