    ...
    <div style={{ position: "fixed", inset: 0, background: "#0b1121", color: "#d4dce9", fontFamily: "'DM Sans',system-ui,sans-serif", overflowY: "auto", zIndex: 9999 }}>
                ...
                <div key={aKey} style={{ background: "#111d33", borderRadius: 12, padding: 12, marginBottom: 10, border: "1px solid #1e2d48" }}>
                  ...
                  {pts.map(pt => { const s = st[pt.id] || "pending"; const ptCava = cavas.find(c => c.atribId === aKey && c.pontoId === pt.id); const prepEq = ptCava ? EQUIPES.find(e => e.id === ptCava.prepEqId) : null;
                    ...
                    <div key={pt.id}>
                      ...
                      {/* Service list when ponto is checked */}
                      {s === "ok" && pt.svcs && pt.svcs.length > 0 && (
                        <div style={{ background: "rgba(34,197,94,.03)", borderLeft: "2px solid rgba(34,197,94,.15)", borderRight: "1px solid rgba(34,197,94,.08)", borderBottom: ptCava ? "none" : "1px solid rgba(34,197,94,.08)", borderRadius: ptCava ? 0 : "0 0 8px 8px", padding: "4px 8px", marginBottom: ptCava ? 0 : 3 }}>
                          <div style={{ fontSize: 8, color: "#5a7aa0", fontWeight: 700, textTransform: "uppercase", marginBottom: 3 }}>Serviços — desmarque os não executados</div>
                          {pt.svcs.map(sv => {
      ...
      {screen === "import" && (
        <div style={{ padding: "14px 14px 100px" }}>
          ...
          <button onClick={() => { if (confirm("Tem certeza? Isso vai apagar TODAS as notas, atribuições, retrabalhos, preparações e inspeções.")) { fbRemove("notas"); fbRemove("atribs"); fbRemove("retrab"); fbRemove("cavas"); fbRemove("preps"); fbRemove("inspecoes"); setImportMsg("Banco resetado! Reimporte os Excel."); } }} style={{ width: "100%", marginTop: 14, padding: "12px 0", background: "rgba(239,68,68,.08)", color: "#ef4444", border: "1px solid rgba(239,68,68,.15)", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🗑️ Resetar Banco Completo</button>
        </div>
      )}
